...
- **P1**(★**총괄 결정 대기 · 권고 있음** · 선행없음) `Jun025/RustJava` **fork 이탈**.
  ★★**[2026-09-03 정정] 「218곳 · 3종」은 «220곳 · 4종»이다** — P2 회차가 격리 워크트리에서
  실제로 `cargo check` 를 돌려 ★**네 번째 파열**을 찾았다: `ClassDefinition::{interface_names, prepare}`
  (`wie_ktf`·`wie_lgt` 의 impl **2곳**). 도입 지점은 ★**`ebd9c03`(+1)·`07fc404`(+2)** —
  ⇒ ★★**§6-P1 이 남긴 「하드닝은 있고 API 파열은 없는 중간 rev」 가설은 «반증»됐다.
  API 안전 rev 는 `bee850f` 자신뿐이다.**
  ★**그 대신 «비용 계단»이 드러났고, 그것이 이 회차의 실질 산출이다**(§8-4):
  `bee850f`(+0) **0곳** → `fe5d116`(+16) ★**2곳**(upstream 커밋 16건 획득) → `5b84dd1`(+33) 5곳 →
  `95ebc5c`(+46) 11곳 → `ba5797b`(+47) ★**220곳**(209 가 «마지막 한 커밋»에 몰려 있다).
  ⇒ ★**P1 은 「218 아니면 0」의 동전던지기가 아니라 «따로 착지시킬 수 있는 계단»이다.**
  ★★**권고(한 줄 · 집행하지 않았다)**: ⒝ 를 고르되 핀을 `bee850f` 가 아니라 ★**`fe5d116`(+16)**
  으로 잡아라 — 측정된 대가는 **2 impl** 이고 그 값으로 upstream 커밋 16건을 산다. 그중
  ★**`8ac70cb`(+5)는 `DataInputStream.readUTF()` 의 EUC-KR panic 수정**이고 ★**우리 fork 는 그 자리가
  아직 `.unwrap()` 이다** — 한국 피처폰 경로에 정확히 얹히는 회귀를 우리가 «갖고» 있다.
  ★**근거의 성격을 흐리지 마라**: **API 축 = 측정** · **코퍼스 축 = 미측정**(P2 가 못 낸다).
