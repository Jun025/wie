...
## 완료 (최근)
- 2026-09-18: **PAT 판정 — 「지금은 쓰지 않는다」 · 다만 «사람이 본다»가 사실상 «안 본다»임을 수로 인정했다**
  (`wie-branch-protection-drift-watch-pat-decision` · 채택 `2026-09-17-branch-protection-claim-guard#p1`) —
  정본 `docs/report/0158--….md`. ★**판정 회차 · 코드 0줄 · PAT 발급 0 · 시크릿 0 · #p0 무접촉**.
  ★**ⓐ 전제를 인용 없이 재확인**: run `35180786771` = **jobs 0 + name 이 원시 경로**(startup_failure 지문 ⇒ `administration:read` 파싱 거부) ·
  run `35181122022` = `pull_request` **failure**(403) ⇒ 「주기 감시에 PAT 가 필요하다」 **참**.
  ★★**판정의 축은 «누가 바꿀 수 있나»다** — `owner.type=User` ⇒ **조직 상위 ruleset 이 존재할 수 없고** `bypass_actors: []`
  ⇒ ★**변경 주체 = 운영자 1인** ⇒ 주기 감시는 대체로 «자기 변경을 자기에게 알리는» 일이 된다.
  ★**대가를 수로**: PAT 는 시크릿 **3 → 4(+33%)** + 만료 ≤1년의 **회전 의무**(주인 없는 의무는 죽는다) ·
  ★**그 이점도 적었다** — 실패가 «만료 → 잡 red」로 **시끄럽고 소유자 규칙이 이미 있다**(드리프트의 침묵과 대조).
  ★★**「사람이 본다」를 미화하지 않았다** — `AGENTS.md` 가 명명한 두 사건 중 ①은 first-parent 전체에서 ★**1회**(`6144f635`)뿐이다
  ⇒ ★**이 판정의 정직한 번역은 «⒝ 로 위장한 ⒞»**이고, 포기하는 것은 **잎 필드 27개(그중 21개는 새 감시분)** 이며 **그 창에 상한이 없다**.
  ★**그래서 뒤집히는 조건을 셋 박았다**: ⑴`owner.type` 이 `Organization` ⑵`bypass_actors` 길이 ≥1 ⑶ruleset 형상 변경.
  ★★**[승계 `-fix` 정정] 초판은 그 셋을 «측정 가능»이라 적었는데 «누가 재는가»가 «셋 다 없었다»** — 그리고 ⑵⑶은
  `rulesets` 엔드포인트를 묻는데 ★**CI 토큰이 못 읽어**(403) 「관측하면 PAT 를 쓴다 · 관측에 PAT 가 필요하다」로 **순환**이었다.
  ⇒ ⑵는 **PAT 불요 대체 신호**로 주인을 붙였다 — ★**형상 가드(PR #197)가 `bypass_actors` 를 이미 비교한다**(실측 **rc=1 · line 14**) ·
  ⑶은 `updated_at ≠ created_at` 이 ★**t=0 에 이미 참**(생성 트랜잭션 **31ms** 차)이라 **상시 오탐**이므로
  **「형상이 바뀌었는데 운영자가 기억하지 못한다」**로 **교체**했다(정규화가 타임스탬프를 버려 그 오탐이 구조적으로 안 난다).
  ★**⑵⑶의 주인은 «#197 착지»가 전제**이고 ★**⑴은 오늘도 주인이 없다**(기계 배정은 후속 제안 `#p1`).
  ★**대가**: 술어 «밖»의 경우(조직 이전 없이 PAT 가 필요해지는 등)는 이 조건이 안 잡는다 ⇒ **그때는 다시 판정한다**.
  ★**그날의 최소 권한 명세도 미리 적었다**(다시 파생시키지 않도록): **fine-grained · 단일 repo · `Administration: Read-only` 하나** ·
  repo secret · 소비자는 **기존 주간 잡** · 만료 ≤1년 · 발급은 **human-step 별건**(이 회차는 발권도 안 했다).
...
- 2026-09-17: **base swap 이 남긴 «고아 파일»을 전수로 셌다 — 4건이고, 3건은 되살리면 red 다(추정 아님)**
  ...
  ★**술어**: cargo metadata 의 패키지 디렉터리를 «소유자»로 두고 추적 `.rs` **316건**을 귀속 ⇒ member 밖 **14**.
  ★★**그 14 중 10 은 고아가 «아니다»** — 루트 패키지 `wie` 소유 **7**(`src/*`·`tests/font.rs` · 소유자가 `.` 이라 단순 술어가 놓친다) ·
  `wie-app` **3**(`Cargo.toml` 이 exclude 를 주석으로 정당화한 **의도된 예외**). ⇒ ★**진짜 고아 = 4**(전부 개명 잔재).
  ★**「있는데 안 돈다」를 기계로 확정**: 4건 전부 **cargo test 타깃 0건**.
  ★★**되살린 것은 «하나»**(계약) — `wie_j2me/tests/test_boot.rs` → `wie-j2me/tests/test_boot.rs` · **내용 0줄 수정**
  (`include_bytes!` 상대 깊이가 같다). ★**「돈다」와 「잡는다」를 둘 다 보였다**: `2 passed` ↔ 제품 호출부
  `wie-j2me/src/emulator.rs` 의 `tick()` 을 `Ok(())` 로(프레임 0장) 개악하면 ★**2 FAILED**, 복원 시 바이트 동일·**2 passed**.
  ★그 개악은 그 파일 머리주석이 지목한 그 갈래다 — 「아무것도 안 던졌다」는 **참인데 프레임이 0장**(2026-09-04 사고의 모양).
  ★★**나머지 3건의 red 를 «추정하지 않고» 쟀다**(제 크레이트로 복사해 실행 후 제거 · 커밋 0):
  `E0061` **2건**(상류 `invoke_virtual` 인자 드리프트) · `E0432` **1건**(`java_class_proto` 의존 미선언)
  ⇒ ★**원인이 갈리므로 한 티켓으로 묶지 마라**(후속 제안에 근거와 함께 적었다).
  ★**검사기는 만들지 않았다** — 제안이 예고한 오탐이 **이 회차에서 실제로 났고**(루트 7 · `wie-app` 3),
  예외 선언 자리를 먼저 정하지 않고 만들면 그 오탐이 굳는다.
  ★회귀: `cargo test --all` **44타깃 · 388 passed · 0 failed**(부활 전 43/386 ⇒ **+1 타깃 · +2 테스트**) · 4게이트 + beta rc=0 · 검사기 8/8 · audit PASSED.
...
- 2026-09-17: **0134 중복은 «이미» 해소됐다 — 그리고 «규칙대로» 해소됐음을 커밋 시각으로 확인했다**
  (`wie-adopt-serial-claim-window-cross-pr-p0` · 채택 `2026-09-17-serial-claim-window-cross-pr#p0`) —
  정본 `docs/report/0149--….md`. ★**코드 0줄** — 제안이 지목한 상태가 이미 사라져 있었고,
  ★**제안 자신의 tradeoff 가 그것을 예고했다**(「이 제안은 그 사실의 기록으로 읽어라」).
  ★**그래도 믿지 않고 쟀다**: #176·#177 **둘 다 MERGED** · 착지본 = #176 `0133`+**`0139`** · #177 `0134` ⇒ 중복 0 ·
  지금 열린 PR 7건 claim(`0116·0137·0142·0144·0146·0147·0148`) **전건 유일** · 디스크 139건 중복 0.
  ★★**「규칙대로였는가」를 커밋 시각으로 판정** — 규칙은 「나중에 claim 한 쪽이 옮긴다」이고 그 축은 **PR 번호와 갈린다**:
  #177 이 `f8b48c99` **08:13:15** 로 **먼저**, #176 이 `115e3665` **08:18:13** 로 **나중**(4분58초 뒤),
  이동 `0aa4a865` **09:45:29** ⇒ ★**옮긴 쪽 = 나중 claim = 규칙 일치**(번호만 보면 #176 이 낮아 «먼저»로 오독된다).
  ★★**「다시 나지 않는가」도 개악으로 확인**(막는 힘은 이 회차가 아니라 이미 착지한 축이 진다):
  ⒜열린 PR **#187** 이 든 `0147` 을 또 집으면 **rc=1**(상대 PR·파일명·이동 규칙까지 찍는다) ⒝디스크 중복이면 **rc=1** ↔ 둘 다 제거 후 **rc=0**.
  ★**남긴 천장 2**: ⑴창은 «조회→커밋»이라 «같은 순간» 커밋은 **둘 다 green** 으로 지나간다(착지 후 축이 받는 **2단 방어**)
  ⑵열린 PR 조회 실패 시 그 축은 **경고로 강등되고 막지 않는다** — 「막는다」를 상시 사실로 읽지 마라.
  ★회귀 0: `cargo test --all` **43타깃 · 386 passed · 0 failed** · 4게이트 + beta **rc=0** · 검사기 **9/9** · audit PASSED.
- 2026-09-17: **upstream 감시 가드를 «지웠는데 green» 이 되지 않게 했다 — 그리고 순진한 술어는 «공허»했다**
  (`wie-adopt-slice-d-base-swap-fix2-p3-p0-p0` · 채택 `2026-09-17-adopt-slice-d-base-swap-fix2-p3-p0#p0`) —
  정본 `docs/report/0148--….md`. ★**제품 동작 0줄 · Rust 0줄 · 기존 검사기 무접촉.**
  `scripts/check-upstream-guard-wired.mjs` 신설 + `engine-contract.yml` **상시 스텝**(paths 필터 «밖» · ~40ms).
  ★**병**: `scripts/check-upstream-new-workflows.mjs` 는 `doc-liveness.yml` 스텝 **하나**로만 돌고,
  그 스텝을 지우면 파일은 남고 전 검사가 green 이다 — 실행 참조가 **하나**뿐임을 전수로 확정(6건 중 1건).
  ★★**설계 급소 — 순진한 포함 검사가 «공허»하다**: 그 워크플로 **머리 주석**(`:34`)이 같은 경로 문자열을 들고 있어
  호출 줄을 지워도 `grep -q` 는 **통과한다**(실측). ⇒ ★**«주석 아닌 줄»에서만 센다.**
  ★★**둘째 결정 — 워크플로 «이름»을 박지 않았다.** `check-parity-lock-wired` 가 2026-09-06 에 스펠링을 박아
  정당한 리팩터를 red 로 만들었고 ★**그것이 가드가 삭제되는 경로**다 ⇒ `.github/workflows/` **전수**를 보고
  어디서 걸렸는지는 «요구»가 아니라 «보고»다.
  ★**개악 3종**: M1 호출 줄만 삭제(주석 존치) **rc=1** ↔ 순진한 술어 **통과(공허)** · M2 가드 파일 제거 **rc=1** ·
  ★**M3 정당한 «이동»(`rust-audit.yaml` 로) → rc=0 GREEN** 이고 보고가 따라간다 — ★**M3 이 핵심 방어다**
  (없으면 이 가드는 다음 리팩터에서 삭제될 종류가 된다). 복원 후 워크플로는 `HEAD` 와 **바이트 동일**.
  ★**남긴 천장 3**: ⑴배선을 보지 **주기**를 안 본다(라이더를 per-PR 로 옮기면 green — 그건 diff 리뷰 몫)
  ⑵**무한퇴행이 남는다**(이 가드 자신은 `engine-contract.yml` 스텝이 잡고, 그것까지 지우면 diff 뿐 — 한 층 더 쌓지 않았다)
  ⑶「어느 워크플로인가」를 안 묻는 느슨함은 ⑴의 대가로 **일부러** 골랐다.
  ★회귀 0(전건 합산): `cargo test --all` **43타깃 · 386 passed · 0 failed** · 4게이트 + beta **rc=0** · 검사기 **9/9** · audit PASSED.
...
- 2026-09-17: **가드는 옳게 섰는데 그 가드가 «자기 가동 방식»을 거짓으로 말했다 — 「주 1회」를 걷어냈다**
  ...
  ★**고친 것은 서술뿐**: 「`…diffs it against the API weekly`」·「`the weekly job grants it`」·
  「주 1회 API 와 대조한다(이 문단이 또 낡으면 ★**그쪽이 운다**)」 → **「손으로 돌린다 · CI 는 못 읽는다」**
  (run `35181122022` 403 → rc=2 · run `35180786771` `administration: read` 는 ★**줄 수 없는 키**라 파싱 거부).
  ★★**왜 major 였나** — ⒜그 거짓이 **가드 머리 주석**이라 다음 사람이 가장 먼저 읽고 ⒝이 가드의 유일한 기동 축이
  **사람의 규율**인데 그 문장이 정확히 그 규율을 해제시킨다(「기계가 본다니 안 돌려도 되겠군」)
  ⇒ ★**이 회차가 고치려던 결함과 «같은 형태»다**(문서가 감시를 실제보다 크게 말하고 아무도 안 본다).
  ★★**전수 조회가 «다섯째»를 찾았다** — 검수가 지목한 4곳 외에 이 PR 자신의 worklog `limits[3]` 이
  「**주 1회 잡의 red 소유자 규칙이 그대로 적용된다**」라고 적고 있었다. ★같은 파일 `limits[0]` 은
  「주기 실행이 «없다»」라고 정확히 적어 두었으니 ★**한 파일이 자기를 반증**하고 있었다.
  ★`weekly`·`주 1회` **34건 전건 분류** ⇒ 남은 거짓 **0건**(`doc-liveness` 자신은 실제로 주 1회라 참).
...
- 2026-09-17: **제안이 시킨 대로 «먼저 쟀다» — 그리고 그 계측기가 «틀렸다»**
  (`wie-adopt-gate3-deploy-selfverify-instruction-p0` · 채택 `2026-09-17-gate3-deploy-selfverify-instruction#p0`) —
  정본 `docs/report/0142--….md`. ★**제품 코드 0줄 · 기계 강제 0줄 신설** · 접촉 = `AGENTS.md` 1곳(±27/8).
  ★**검사기를 «만들지 않은 것»이 산출물이다** — 제안 자신이 「먼저 «문서만으로 얼마나 고쳐지는지»를
  다음 33건으로 재라」로 처방했고, 그 측정을 했더니 ★**측정 도구가 답을 낼 수 없었다.**
  ★**두 방향 반증(전부 실측)**: ⑴오늘 `#177`·`#178` 을 착지시킨 게이트③ 두 회차는 브라우저 검증을
  **실제로 돌렸고**(`rc=0` · `console errors 0`) `WIE_BASE` 를 인용하는데, 리터럴 `Deploy to Cloudflare Pages`
  를 안 써서 ★**분자·분모에서 «둘 다» 빠진다** ⇒ 준수가 올라도 비가 안 움직인다.
  ⑵★`web.yml` 의 `push` 트리거에 **`paths` 필터가 없다** ⇒ ★**이 repo 엔 «배포 없는 착지»가 없다**
  (최근 머지 회신 12건의 `merged:` sha 에 `Web` 런 조회 → **12/12**) ⇒ 「107 착지 중 36 만 배포 수반」은 **구성상 거짓**.
  ★**수 교체**: 어휘 분모 **36/18 = 50%** ↔ ★**사실 분모(실제 착지) 107/57 = 53%** — 표로 넣고 「2행을 읽어라」를 명시.
  ★**조건절 제거**: 「배포를 수반하면」 → ★**「착지하면 언제나」**(4-C 의무가 이 repo 에선 «무조건»이다).
  ★**축을 남겼다**: 검사기를 만들거든 **`merged:` sha + GitHub 런 목록**으로 — ★**회신 문구로는 절대 안 된다.**
  ★**대가**: 게이트③ 회차마다 브라우저 실행 1회가 **무조건** 붙는다(사실에 맞지만 비용은 실재).
...
- 2026-09-17: **「대조 안 했음」이 「대조했고 깨끗함」으로 읽혔다 — 강등을 «성공 줄»이 말하게 했다**
  ...
  ⇒ 재현(`gh` 실패 shim + 실제 충돌): 수정 전 「충돌 0」 ↔ 수정 후 **「★미대조(열린 PR 을 못 물었다)」**.
  ★**fail-open 은 유지**(경고 · red 아님) — 고친 것은 판정이 아니라 **«무엇을 말하는가»**다.
  ★**F2**: 자기 제외를 «파일명»에서 **«head 브랜치»**로(파일명은 2차) — 연번 고정 + 슬러그만 바꾸면 종전엔
  ★**자기 PR(#179)과 충돌**해 적용 불능 규칙을 찍었다(재현: 전 rc=1 ↔ 후 rc=0).
  ★★**F3 — 규칙의 «축»을 바꿨다**: 「PR 번호가 큰 쪽」 → ★**「나중에 claim 한 쪽이 옮긴다」**.
  체크런이 커밋에 묶여 **늦게 claim 한 쪽이 먼저 빨개지므로**, 번호 축이면 «통보받은 쪽은 멈춰야 하고 움직일 쪽은 모른다».
  ★**두 축이 실제로 갈린 사례가 있다** — `0134` 에서 #177 이 먼저 claim 했고 번호는 #176 이 작은데, 게이트③이 옮긴 것은 **#176** 이다.
  「두 PR 이 다 red」도 **「각자 다음 run 부터」**로 고쳤다. ★**F4**: `origin/main` ref 의존을 워크플로에 **선언**(주석 · 구조 변경 0).
  ★★**대조군이 이 회차가 새로 넣은 버그를 잡았다** — jq 템플릿을 JS 문자열에 넣어 `\(`·`\t` 가 먼저 먹혀 **모든 조회가 실패**했고,
  ★그 증상이 정확히 **F1 의 새 문안**으로 드러났다(고치기 전이었다면 「충돌 0」으로 조용히 지나갔다) ⇒ `JSON.parse` 로 교체.
- 2026-09-17: **연번 충돌이 «착지 후»에만 울었다 — 열린 PR 끼리도 울게 했다**
  (`wie-adopt-slice-d-base-swap-fix2-p1-p0` · 채택 `2026-09-16-adopt-slice-d-base-swap-fix2-p1#p0`) —
  정본 `docs/report/0136--….md`. 새 잡·새 도구 **0**(기존 `openPrAddedNames()` 재사용 · 기존 스텝 그대로).
  ★**제안이 지금도 참인가 — 참이고, 이 회차 «도중에» 다시 났다**: `0134` 를 **#176·#177 이 둘 다** 들고 있다
  (#177 커밋 `23:13:15Z` → PR `23:13:41Z` ↔ #176 커밋 `23:18:13Z` = ★**5분 늦게, 이미 남의 번호였다**).
  ★★**제안의 처방은 «쟀더니» 기각됐다** — 「claim 조회를 방금 push 된 브랜치까지 넓혀라」가 사는 시간은
  **커밋→PR 간격**인데 PR **32건** 실측 **p50 28초 · max 70초**다. 실제 충돌은 **5분·8분** 간격 ⇒ ★**넓혀도 못 막는다.**
  ★**진짜 창은 «조회 → 커밋»(회차 작업 시간)** ⇒ 고친 자리는 조회가 아니라 ★**«커밋 시점에 이미 잡혀 있는가»**이고,
  그 축은 회차가 커밋 전에 돌리고 CI 가 PR 마다 돌리는 **같은 기본 모드**다.
  ⇒ 기본 모드가 `crossPrCollisions` 로 **exit 1** 을 내고 ★**«누가 옮기는가»까지 찍는다** —
  ★**나중에 claim 한 쪽이 옮긴다**(체크런은 커밋에 묶여 있어 «늦게 claim 한 쪽»이 먼저 빨개지고 먼저 claim 한 쪽은
  자기 CI 가 다시 돌기 전까지 모른다 ⇒ **통보받은 쪽 = 움직일 쪽**이 되게 축을 맞췄다). ★**오탐을 막는 자리**: 대조 대상은 «디스크 전체»가 아니라 **디스크 − `origin/main`** 이다
  (전체로 대조하면 **남의 두 PR 충돌이 내 PR 을 red 로** 만든다). ★`main` push 는 더한 것이 0이라 **네트워크 0**.
  ★★**양방향 실증(실데이터)**: 내 파일을 `0134` 로 두면 **rc=1** 이고 `::error` 2줄이 **#177·#176** 을 이름으로 지목 ↔
  `0136` 이면 **rc=0**. ★대조 상대가 상수가 아니라 **지금 열려 있는 실제 PR** 이다. `--selftest` **15케이스** 전건 일치.
  ★**대가**: PR 빌드가 네트워크를 친다(실패 = 경고 · red 아님 · ★그 대신 **성공 줄이 «미대조»라고 말한다**) ·
  충돌 시 **각자 다음 run 부터** 두 PR 이 red(★«즉시»가 아니다 — 체크런은 커밋에 묶인다) ·
  CI 배선 2줄(`pull-requests: read` + `GH_TOKEN`)이 빠지면 ★**이 축이 조용히 경고로 강등**된다.
...
- 2026-09-17: **연번 0113 이중 claim — «이미 해소됐다»를 «처분 기록»으로 남긴다**
  ...
  ★**사실은 그대로다**: #159(조각 C)가 `0113` 으로 먼저 착지하자 #160(조각 A)의 `-fix` 회차가 커밋 **`39ffb2ef`**
  로 **착지 전에** 자기 번호를 `0113 → 0115` 로 옮겼다 — `AGENTS.md` 의 「착지하지 «않은» 쪽을 옮겨라」 그대로이고
  ★**제안이 예측한 번호(`0115`)까지 맞았다.** 지금 `main` 의 `0113` **1개** · 열린 PR claim **0** · 검사기 **rc=0**.
  ★★**반려된 것은 «판정»이 아니라 «기록 부재»다** — 직전 회차가 Acceptance ⑶ 을 「worklog 를 낳지 않는 회차라 해당 없음」
  으로 접었는데 그것이 ★**순환**이었다. 형제 `#p0`·`#p2` 는 **같은 계급의 결론**(「이미 났다」·「이미 지켜졌다」)에서
  각자 `adoptedProposals` 를 **기록했고 착지했다**. ★`#p0` 리포트가 그 이유를 명시해 두었다 —
  「★**이 회차의 진짜 산출물은 worklog 의 `adoptedProposals` 한 줄**이다」.
  ⇒ ★**Contract 2 의 «코드 0줄»은 «파일 0개»가 아니다** — 처분 기록은 코드가 아니다.
  ★**파생**: ⑷⑸(PR head·`ci-presence`)를 「PR 이 없으니 해당 없음」으로 접은 것도 ★**자기가 만든 전제로 자기를 면제한 것**이었다.
  ★**표제도 정정했다** — 직전 「PR 0」이 ★**PR #174 를 `CLOSED` 로 닫은 저장소 상태 변경**을 덮었다(본문엔 전문 공개돼
  은폐는 아니고, 브랜치 `wie-worklog-coverage-remeasure-104`(`9dc95560`) 원격 보존이라 **reopen 가능**).
- 2026-09-16: **「조각 D 재발권은 #159 뒤로」는 «이미 지켜졌다» — 다만 #159 의 가드는 지금 돌지 않는다**
  ...
  ★**순서 실측**(원격 조회): `#159` MERGED **01:20:39Z**(`3744f84e`) · 조각 D 재발권분 `#161` MERGED
  **08:35:30Z** ⇒ ★**#159 가 먼저 · 간격 7:14:51**. ★**실익도 성립**: #159 가 지운 **7파일**이 오늘 main 에
  **전건 부재**(`git cat-file -e` 잔존 0) ⇒ 조각 D 가 그것을 손으로 다시 화해시킬 일이 없었다.
  ★**제안의 「8건」 ↔ 실측 「7건」은 모순이 아니다** — 센 대상이 다르다(교집합 ↔ 삭제 수).
  #159 가 `wie_wipi_java/` 에서 만진 것은 전수 **10건**(`D` 7 · `M` 2 · `A` 1) ⇒ **삭제 7 + 수정 1 = 8**.
  ★★**못 잰 것은 «못 쟀다»고 적는다** — 조각 D 가 기록한 **74→67** 충돌 계수는 **오늘 재현 불가**다
  (그 수는 당시 merge-base 기준이고 base 가 여러 번 움직였다). ⇒ 순서·실익을 **착지 시각**과 **파일 부재**로
  대체 측정했고 그 둘은 오늘도 참이다. ★**재현 불가를 «확인됨»으로 적지 않았다.**
  ★★**부수 관측(범위 밖 · 고치지 않았다)**: #159 가 자기 개악 대조로 세운
  `wie_wipi_java/tests/preload_classes_come_from_the_runtime.rs` 는 **파일로 실재**(3,136B)하나
  ★workspace member 가 `wie-wipi-java`(하이픈)라 `wie_wipi_java`(언더바)는 **member 가 아니고**
  (`grep -c '"wie_wipi_java"' Cargo.toml` → **0**), `cargo test --all` 출력에 그 이름이 ★**0회**다
  ⇒ ★**존재하지만 실행되지 않는 가드**다. 조각 D 가 이미 「고아 5파일 · 조각 E 소관」으로 지목한 축이라
  **손대지 않았고** 후속 제안으로 뺐다.
  ★**진짜 산출물은 worklog 의 `adoptedProposals` 한 줄** — 형제 `#p0` 이 **집행 4시간 30분 뒤 채택된** 실례가 있다.
...
- 2026-09-16: **아티팩트 다운로드 예산을 계약에 핀했다 — 상한을 넘는 두 사건이 «같은 창»에 들어왔다**
  ...
  ★**제안이 «내용»이라 한 선택(절대 바이트 ↔ 증가율)을 실측으로 골랐다**: `web/.gitignore` 가 `src/wasm/` 를
  무시해 ★**아티팩트가 git 에 없다** ⇒ 「전 커밋 대비」는 두 번 빌드(1회 **5분 55초**)하거나 기준 수를
  저장해야 하는데 ★**후자가 곧 절대 핀**이다 ⇒ 증가율 축은 **성립하지 않는다**.
  ★★**그리고 제안이 «약점»으로 적은 wasm-opt 흔들림은 반대로 «잡아야 할 것»이었다** —
  `build-wasm.sh:41-53` 의 폴백(wasm-opt 부재·구 binaryen 파싱 실패)이 ★**25,591,056B = 최적화본의 1.66배**를
  내고 `publish-artifact.yml` 이 **같은 스크립트**를 부른다 ⇒ **폴백판이 그대로 릴리스된다.**
  ★**상한 `20,000,000` 은 «창»이다**: 오늘치(**15,425,955**) 위 **29.7%** · 폴백판 아래 **21.8%**.
  ★**개악 대조는 «제품 경로 실주행»이다**(상수 대 상수 아님) — 실패하는 `wasm-opt` 를 PATH 앞에 놓아
  폴백 분기를 실제로 태웠다 ⇒ **rc=1 `over budget 128.0%`** ↔ 정상 **rc=0 109 pass** ↔ 복원 **rc=0**.
  무예산 파일은 **fail-closed 위반**(Constraint 3). ★**한계**: 수는 **로컬 binaryen 130** 으로 쟀고
  **CI 는 119 핀**이라 ★**CI 값은 이 회차가 못 쟀다** — 이 PR 의 `engine-contract` 잡이 처음 실측한다.
...
- 2026-09-13: **문서↔계약 어긋남 경보 — 판정 «만들지 않는다(⒥)»**
  ...
  ★**그래도 만들 값이 없다**: ⑴★**CI 가 그 계약을 볼 수 없다** — 워크플로 ★**9/9** GitHub 호스티드 · self-hosted **0**
  ⇒ 게이트가 못 되고 «로컬 전용»이면 **매 회차가 기억해야 하는 것**이 하나 더 늘 뿐이다(없애려던 실패 양식의 재생산).
  ★★**이 한 다리만으로 ⒥ 가 선다 — 아래 수들과 «무관»하다.**
  ⑵★★**[`-fix` 철회] 초판의 둘째 다리 「참조 쪽 추적성이 없다」는 «거짓»이었다** — orchestrator `origin/main`
  재측에서 두 파일 **모두 추적**되고 `templates/merge-ticket.tpl` 은 **1,228줄 · `4-A` 11건**이다.
  ★**내가 «낡은 로컬 체크아웃»을 정본으로 착각했다**(`~/orchestrator`@live · HEAD `13b88a0c` · **2,476 커밋 뒤** ·
  거기선 커밋본 **63줄·`4-A` 0건** · conf 는 **untracked**). ★**그 트리 수치는 지우지 않는다** — 「로컬 전용
  검사기가 실제로 무엇을 읽게 되는가」의 실측 예시다. ⇒ 남는 위험은 「추적 불가」가 아니라
  ★**「머신마다 다른 바이트를 읽는다」**로 좁혀진다.
  ⑶★★**[`-fix` 정정] 지킬 표면을 repo 전수로 다시 셌다 — «값 복사» 2 → ★7**(초판은 `AGENTS.md`·`CLAUDE.md`
  **두 파일만** 봤다). ★**세는 기준**을 못박았다 = **살아 있는 지시문만 센다 · 이력은 뺀다**(이력 **45**건 =
  `STATE.md` §완료 21 + `docs/report/**` 24 — 스냅샷이 정본이라 고치면 **사료가 거짓**이 된다).
  ★그중 ★**«현행 어긋남» 1건** — `CONTRIBUTING.md` 가 **여전히 `--squash --delete-branch` 를 지시**하고 있었다
  ⇒ ★**이 회차가 고쳤다**(두면 이 판정이 「사람 규율로 충분」의 **반례**를 안고 닫힌다).
  ★`AGENTS.md` 의 값 복사 2 는 «같은 계약»이되 ★**«다른 두 절»**(§DoD ↔ §Git Workflow)이다 —
  초판의 「같은 불릿」은 틀렸고, ★**두 절이면 한쪽만 고쳐도 나머지가 남는다**(9일 모순이 그 형태였다).
  ⇒ 더 싼 대안(**값을 복사하지 말고 정본을 가리켜라**)이 이미 착지해 **표면을 줄였다**.
  ★**단 ⒥ 도 공짜가 아니다** — 절 단위 포인터(`§4-A`)는 ★**조용히 썩는다**(정본이 renumber 해도 신호가 안 온다).
  ★남는 방어선은 사람 규율 한 줄이고 **자리까지 정했다**(`AGENTS.md` 「Held by you, not by a machine」) —
  ★**이 회차가 쓰지는 않았다**(그 파일 재수정이 범위 밖) ⇒ 제안 `#p0`.
  ★**재개 조건**: self-hosted 러너 도입 · 값 복사 **10곳 이상**(오늘 7) · 계약 파일의 repo 내 벤더링 —
  ★**셋째는 «가정»이 아니라 이 repo 가 «이미 쓰는 수법»이다**(`check-engine-contract.mjs` 가 otterpebble 소비
  계약을 벤더링해 검사한다). 정본 = `docs/report/0109--….md`.
...
- 2026-09-10: **배치 결정 — 문서 명령 생존성 검사는 ⒞ «주 1회 예약 잡» 하나다(구현은 별 회차)**
  ...
  ★근거: 빈틈(러너 블록 3 + **별칭 층 6**)을 전부 담을 자리는 부작용 명령이 허용되는 예약 잡뿐이고,
  러너 블록 뒤 기능 회귀는 E+F 가 이미 두 겹으로 잡는다(2026-09-07 기결정) ⇒ 남는 «명령 표면 낡음»은 주 1회 지연이 맞는 가격.
  ★별칭 6건 처분 = ⒞가 덮는다 — 잡은 문서 **문면 그대로(별칭)** 를 돌아야 한다(실체 호출 금지 · 설계 요건으로 명시).
  ★⒞의 「주인 없는 red」는 소유자 규칙을 **잡이 생기기 전에** AGENTS.md 에 박아 셈했다(빨간 주간 런 뒤 첫 게이트③ 회차).
  기각 1줄씩: ⒜ 커밋당 413s+ 지불 불가·별칭 미커버 / ⒝ E+F 중복 + 2026-09-07 이 기각한 가격 + 필터 침묵(#130) /
  ⒟ 주당 87~218분 > 주 1회 런 + 작업 트리 쓰는 손 실행(⒜ 기각 사유와 동형). 정본 = `docs/report/0101--….md`.
...
- 2026-09-09: **`smoke_gate.sh` 의 JSON 읽기도 «같은 안전 술어»로 — ★여기선 «거짓 PASS» 가 가능했다**
  ...
  ⇒ 반복 키에서 뒤엣것이 이기면 ★**FAIL 인 run 을 «PASS» 로 보고**한다(게이트가 거짓 green).
  실증: 두 문서·중첩 둘 다 ★**옛=PASS(거짓) → 새=FAIL** + stderr 경고 · 정상 단일 문서 판정 **2/2 불변**.
  ★**빈 파일 함정**(티켓 지목): 빈 파일·키 부재 둘 다 **FAIL**(fail-closed) + 새 코드는 「0건」을 말한다.
  ★**공허하지 않음**: `run_one` 을 **스텁 BIN + bash 3.2 로 실제 구동**(게임 바이트 0) — 네 경우 전건 기대대로.
  ★**술어는 복사하지 «않았다»** — `result` 는 문자열이라 문자클래스·`cut` 구분자가 다르다(제안 `tradeoff` ⑵ 가 예고).
  ★**남는 한계**: `game_lab/`(gitignore·실게임 바이트)를 읽는 **로컬 전용**이라 **CI 검증 불가** — 합성 JSON + 스텁으로만 댔다.
- 2026-09-08: **파리티 «천장 목록»을 한 벌로 — 정본은 «검사가 인쇄하는» 쪽이다**
  ...
  ★**양방향 차집합 = 코드에만 0 · 사본에만 2**(⑧순서·중복 · ⑨줄끝/CRLF) ⇒ 제안의 「2항」 가정 **확인**.
  ★★**문면 갈림도 2건 잡았다**: ⑤는 **사본이 정확**(`:317` 이 `norm(strip_comment(...))`) ⇒ 정본을 그 값으로 정정 ·
  ③은 **코드가 최신**(2026-09-06 「넓히지 않는다」 결정 승격분) ⇒ ★**그 결정 무접촉**(diff ± **0줄**).
  ★**정본 = 코드**(⒜): 검사가 «매 run 에 인쇄»하므로 green 을 읽는 사람과 목록을 읽는 사람이 같은 것을 본다
  ⇒ 갈림이 «구조적»으로 막힌다. ⒝(worklog 정본)는 ★**검사기가 안 읽는 곳이 정본이 되어 갈림이 재발**하므로 기각.
  ★**사본은 지우지 않았다** — `ceilings` 9항 무접촉 + 「스냅샷이지 정본이 아니다」 **1키**만 더했다.
  ★**개악 대조**: M-A(드리프트) → `dod_region_and_rust_yml_agree` **FAILED**(10/1) · 원복 **11/0** ⇒ 기존 축 여전히 red.
  ★그 실패 출력에 **정본 선언·⑧·⑨ 가 실제로 인쇄**됐다.
- 2026-09-08: **`lgt_render_probe.sh` 의 `field()` — ★제안이 지목한 결함은 «없었고», 옆에 «진짜»가 있었다**
  (PR 개설 · `wie-render-probe-field-prefix-collision-safety` · 채택 제안 `2026-09-06-validate-last-frame-richness#p1`)
  — `field()` **하나**(13+/1−). ★**`.rs` 0줄 · 새 의존성 0 · 새 워크플로·잡 0.**
  ★★**기전 반증**: 술어 `"$1":` 가 이름 앞뒤를 «따옴표·콜론»으로 이미 앵커한다 ⇒ `last_frame_content` 는 `"content":` 를 만족 못 한다.
  심어서 확인 — 접두 쌍 5종(순서 양방향 포함)에서 ★**옛 코드도 5/5 정답** ⇒ **접두 충돌 쌍 «0»**.
  ★★**그러나 관찰은 옳았다** — 「마지막으로 맞는 것을 집는다」= `tail -1` 이고, ★**진짜 기전은 «같은 키의 반복»**이다:
  중첩 `{"summary":{"paints":10},"frames":{"paints":3}}` 에서 ★**옛 3(틀림) → 새 10(맞음)** · 키 부재도 이제 **«0건»이라 말한다**.
  ★**처방은 «고르기»가 아니라 «말하기»**: 매치 수가 1이 아니면 stderr 로 알리고 값은 첫 매치로 결정론화(진단 스크립트라 abort 시키지 않는다).
  ★**의존성 결정 = ⒞ 기존 관용구 유지** — `smoke_gate.sh:76` 이 같은 `grep+cut` 을 쓴다(하나가 둘보다 낫다).
  `jq` 는 로컬·CI 둘 다 실재하나 ★**이 스크립트가 CI 밖(로컬 전용)**이라 기여자 머신 보장이 없어 버렸다.
  ★**위양성 0**: 실측 JSON 에서 `paints`·`content`·`distinct_colors` 값 **3/3 불변** · bash **3.2** 실구동 확인.
  ★**남는 것**: `smoke_gate.sh:76` 에 «같은» `tail -1` — 범위 밖이라 두었고(오늘 무증상) 제안으로 올렸다.
...
- 2026-09-08: **되돌림 문서의 «한시 부록»이 소진됐다 — 지웠다** (PR **#136** 착지 ·
  ...
  (diff **−99**) · **§6 소진 기록 16줄** 신설. ★**§0~§5 무접촉 · 되돌림 실행 0.**
  ★**조건을 직접 재확인**: `#98 9533e882`·`#99 b0a08d73`·`#100 53336bae`·`#105 7fa02f16`·`#106 e50eecd4`
  **전건 착지** + `gh pr list --state open` ★**0건**(07:41 KST).
  ★**판단의 축은 «문서의 성격»이다** — 이 문서는 제목·머리글이 스스로 **「되돌리는 절차(명령)」**
  이라 못박은 **런북**이고, 부록만이 되돌림 단계가 «아니었다»(이관 직후 충돌 해소 레시피).
  ⇒ ★**런북이 담으면 안 되는 것은 «실행 불가능한 절차»** ⇒ ⒜ 채택. ⒝(소진 표시만)는 그 기록의 값을
  **`docs/report/0052` 가 이미 진다**는 이유로, ⒞(요약 존치)는 **원문과 어긋날 새 표면**을 만든다는 이유로 기각.
  ★**복구를 «실행해» 확인했다**: `git show b0a08d73:docs/report-migration-revert.md`(223줄 · 부록 97줄) · 신설 `01ef775b`.
  ★★**잔여 위험 «0 이 아니다»** — 원격 7개 전수 측정에서 `chore/claude-autonomy-hardening` 이 아직 이관 전
  형상이다(절 3 · 292 뒤 · PR **#45 CLOSED**). 나머지 5개는 `REPORT.md` 파일 자체가 없다. ⇒ §6 에 적었다.
  ★**F3**: 만료 선언 절은 **부록 1건뿐**이고, 나머지 **7곳**은 「이관 시점 값이니 다시 재라」류다.
  ★★**[정정 · 게이트②] 그 7곳이 「그대로 남는다」는 «거짓»이다 — 생존은 «4/7»**: 3곳(`:130`·`:196`·`:214`)이
  부록 범위(127~223) **안**이라 ★**부록과 함께 지워졌다**(삭제 전 실측 8 = 선언 1 + 상시 7 · 안 3 / 밖 4 · 삭제 후 생존 4).
  ★**근인은 삭제가 아니라 «분류»다** — 술어가 «전역 상시 지시»(밖 4)와 «부록 레시피에만 매인 지시»(안 3)를
  한 칸에 넣었다. 뒤엣것은 애초에 부록과 **수명을 같이한다** ⇒ ★**「만료하지 않는다」는 선언이 틀렸다.**
  ★**되살리지 않는다(의도적으로 접었다)**: `:130` 의 5건 충돌 표는 `REPORT.md` 에 **더 상세히** 살아 있고,
  `:196`·`:214` 는 **레시피의 술어를 지키는 지시**라 레시피 없이 복원하면 참조 대상이 없다 —
  함정의 «이름»은 `docs/report/0052` 에, 「한쪽 통째 채택 금지」 일반형은 `AGENTS.md` union 규약에 있다.
  ★전문이 필요하면 **§6 의 복구 명령 한 줄**이 부록째 되살린다.
- 2026-09-08: **§완료에만 있던 회차를 `docs/report/` 로 백필 — 「7」이 아니라 «5» 였다**
...
- 2026-09-08: **§완료의 «맨 위 삽입 지점» — ★그대로 둔다(⒞) · 남은 충돌은 «알고 남긴 값»이다**
  ...
  ★★**제안의 수를 둘 정정했다**: ⒜전향 충돌률은 **3/10 이 아니라 1/1**(독립 분기 두 회차가 §완료 머리에 쓰면 항상 CONFLICT ·
  1줄만 떨어지면 clean — 간격 1·2·3 전건 rc=0) ⒝★**`docs/report/` 는 상위집합이 아니다** — 87항목 중 **7건**이 다른 데 없다.
  ⇒ ★**그 7건이 §진행중과의 «성격 차이»의 실체다** — §진행중은 낡은 사본(잃은 서술 0)이었고 §완료는 사본이 아니다.
  ★**⒝(분산)는 측정으로 기각**: 필요한 간격은 «동시 회차마다 1줄»인데 하루 **10~25건** 착지라 다투는 회차가 같은 날에 있다.
  ★**비용 실측**: 추적 344파일에서 `## 완료` 파서 **0** · `STATE.md` 언급 **1**(산문) ⇒ ★**어느 게이트도 막지 않는다.**
  남는 위험은 «조용한 소실»이라 union 의 양방향 보존 계수가 필수다. ★되돌릴 신호 3종을 `AGENTS.md` 에 함께 적었다.
- 2026-09-08: **대장 A 가 낡는 것을 «기계»가 말한다 — 「수」가 아니라 「집합」으로** (PR **#137** 착지 ·
  ...
  ★**새 스크립트 0 · 새 워크플로 스텝 0 · `cargo audit` 호출 «추가» 0**(기존 하나를 나눠 쓰므로
  두 검사가 «무엇이 경고인가»에서 어긋날 수 없다) · ★**의존 무접촉**(lock/toml **0줄** · 억제 추가 **0**).
  ★★**⒝(집합 동등)를 골랐다 — ⒜(수 동등)는 «선행 사고를 못 잡는다»**: 2026-09-06 사고가 **계수 2↔2 로 같고
  구성만 달랐다**. 개악 **M3**(chacha20 → 다른 크레이트 «교체»)가 그것을 재현한다 — ★**`§A 행 2건 · 경고 2건`
  «계수 동일»인데 rc=1**. ⇒ ⒜였다면 침묵했다.
  ★**양방향 개악 전건 실행**: M1 경고 +1 → rc=1 · M2 A 행 −1 → rc=1(+「절 머리 2건인데 실제 1건」) ·
  M3 계수동일·집합상이 → rc=1 · ★**M4 파서를 눈멀게 하면 «조용히 통과»가 아니라 rc=1「못 쟀다」**(fail-closed) ·
  M5 C 행은 A 로 세지 않는다(행 2건). ★`cargo audit` 출력은 **PATH shim 으로 «합성»**했다 —
  ★**스크립트 무수정 · 의존 무접촉**이고, 진짜 JSON 을 먹인 대조군은 rc=0 이다.
  ★★**«A 행»의 정의를 적었다**(불변식의 절반): 줄머리 `| **A-<n>** |` **만** — C 로 옮긴 해소분은
  `| **C-2**<br>(구 **A-2**)` 라 **세지 않는다**(C 절 자신이 「A 의 불변식 밖」이라 못박는다).
  ★**⒞(C 행 정합)는 기각** — 「너무 일찍 C 로 옮겼다」는 ⒝ 가 이미 잡고, 남는 축은 C 표의 **다른 열 순서**를
  파싱해야 해 표면만 는다. 도달성 판정 기계화는 **범위 밖**.
  ★★**required 승격 «안 했다» — 할 수도 없다**: `rust-audit.yaml` 트리거는 **`schedule`+`workflow_dispatch` 뿐**
  (실측 · `pull_request`·`push` 없음) ⇒ PR 에 보고되지 않는다. ★**대가**: PR 이 표를 잘못 고치면 **최대 24h 조용**하다
  (손으로는 `node scripts/check-audit-warnings.mjs` 한 줄). ★**PR 트리거를 일부러 안 붙였다** — 이 병은
  **시간 구동**이다(`chacha20` 은 **lockfile 변경 0**으로 상류가 yank 해서 생겼다).
  ★**지금 검사는 조용하다**(rc=0 · 양방향 차 0) — 직전 회차가 방금 맞췄으므로 ★**표의 «행»은 0건 수정**했다.
...
- 2026-09-08: **「진행중」의 «맨 위 추가»를 없앴다 — 병은 «맨 위»가 아니라 «지점이 하나»였다**
  ...
  ★그중 **`STATE.md` «만»** 10/10(`REPORT.md` **0** — 09-07 이관이 실제로 값했다).
  ★★**결정적 실측**: git 3-way 는 두 삽입 지점이 **1줄만 떨어져도 clean**, **같은 지점이면 위든 아래든 CONFLICT**.
  ⇒ ★**⒜「맨 아래 append」는 효과가 «0» 이다**(4셀 모델 전 셀 2/2 충돌) · ★⒝「PR 번호 정렬」도 무력
  (형제 회차의 번호가 **연속** — #120~#133) ⇒ **⒞(지점을 없앤다)** 를 골랐고, `REPORT.md` 의 **선례를 그대로** 따랐다.
  ★**대조군 4셀**: C0 **10/10** · C1(완료만 제거) **7/10** · C2(진행중만 제거) 3/10 · C3(둘 다) ★**0/10**
  ⇒ ★**제안의 「완료를 갈라도 그대로다」는 대체로 맞지만 전부는 아니다**(**3/10** 은 완료만으로 풀린다) —
  ★**정확한 형태는 «둘 다 지점이고 어느 하나도 충분하지 않다»** 이다.
  ★★**7 과 3 은 «분할»이라 따로 인용하면 안 된다**: 서로소이고 `7 + 3 = 10` · C1 clean 집합 = C2 잔여 집합
  (`6ed4ee8e 480e8654 c2c9552d`). ★`C3=0` ⇒ 충돌은 두 절 «안»에만 있고 `C0=10` ⇒ 두 절 모두 무충돌인 쌍은
  없다 ⇒ 포함배제로 교집합 **0**. ⇒ ★**자기 검산 `C1_clean + C2_clean ≤ 10`**(서로소일 때만 등호) —
  ★초판이 C1 의 clean 을 **4** 로 적었고 `4 + 7 = 11 > 10` 이라 ★**측정하지 않고도 틀렸음이 드러났다**
  (게이트② 반증 · `-fix` 회차가 재측해 바로잡았다 · 스텁 0줄·1줄 동일값).
  ★**F3**: 열린 PR **#98·#99** 를 격리 워크트리에서 **순서 바꿔** 실제 `git merge` — 전 **둘 다 CONFLICT** → 후 ★**둘 다 clean**.
  과거 10쌍은 ★**10/10 → 3/10**. ★★**0 이 아니다** — 잔여 3건의 첫 충돌 줄은 ★**3/3 모두 `## 완료 (최근)` 안**이고,
  그 처분은 이 회차 범위 밖이라 **후속 제안**으로 올렸다(`#114` 의 「완료 분할 안 함」은 뒤집지 않았다).
  ★**정보 보존**: §진행중 항목 **1 → 0** — 그 1건은 회차 서술이 아니라 `gh pr list` 의 **낡는 사본**이었다
  ⇒ **잃은 서술 0** · §완료 **83건 무접촉 · 순서 변경 0**.
  ★★**[정정] 낡는 «방향»이 초판 서술과 반대다**(게이트② N3 · `-fix` 회차 재측): 「직전 판본이 이미 착지한
  #100 을 달고 있었다」는 `main` 에서 **재현되지 않는다**(#100 은 자기 착지 커밋 `53336bae` 에서 정확히 빠졌다).
...
- 2026-09-08: **richness 3축의 «마지막 프레임» 짝** (PR **#100** 착지 · `wie-validate-richness-three-axes-lack-last-frame-pair`
  · 채택 제안 `2026-09-06-validate-last-frame-axis#p1`) — 세 축이 전부 `fetch_max`(단조)라
  `saw_content` 와 **구조적으로 같은 사각**을 갖는다(대전제 ⓐ 재측: `:185`·`:188`·`:189`·`:190`).
  ⇒ 같은 지표를 **마지막 프레임에만** 적용한 `last_frame_*` 3필드를 더했다 — ★**run 당 1회** · ★**보고 전용 · 임계 0 · `passed` 무접촉.**
  ★★**개악 없이도 이미 갈린다**: `keydraw_* --inject` 무개악에서 **max nd=2.1 ↔ last nd=1.1**
  (키를 오름차순으로 눌러 «가장 넓은 막대»가 MAX 에 남지만 마지막 프레임엔 마지막 것만 있다).
  ★**Acceptance**: 마지막 프레임이 초라해지는 개악(LGT 검은 화면 회귀)에서 **max 2/2.1 «불변» ↔ last 1/0.0 «바닥»**,
  대조군 KTF 불변. ★단 `result` 는 두 형상 다 **PASS** — 임계를 두지 않았기 때문이고 그것이 이 회차의 범위다.
  ★소비자 실측 **1개**(`lgt_render_probe.sh`)를 실행으로 회귀 0 확인. 기존 3축·5픽스처 판정 **불변**.
  ★★**[2026-09-07 `-fix`] 착지 충돌을 «저작»으로 해소했다** — `origin/main` behind **116** 을 당겨 `wie_validate.rs` 충돌 **2구간** 처리.
  ⒜**모듈 헤더**: #96 이 «일부러» 고친 `REPORT-ONLY BY DEFAULT …` 를 살리고 옛 문장은 **되살리지 않았다**
  (합집합이면 착지본이 「deliberately **not** a gate」와 「flag 면 gate」를 동시에 주장한다) — ★검산 `deliberately not a gate` **0건**.
  ⒝**`run()` 꼬리**: ★**기록(richness 3필드) → 판정(gate)**. 근거는 코드 — 게이트는 **bool 셋만** 읽고 richness 를 **안 읽는다**
  ⇒ 오늘은 두 순서가 «교환 가능»하고, 그래서 **다음 편집이 0 을 읽는 갈래**로 빠지지 않게 기록을 앞에 뒀다.
  ★**시험으로 잠갔다**(`richness_is_recorded_before_the_gate_judges_test`) — ★**소스 순서를 읽는 시험이고 그것이 의도다**
  (교환 가능하므로 «어떤 행위 시험도» 두 순서를 구별 못 한다). 되돌리면 **FAILED** · 원복하면 **ok** 를 실증했다.
  ★**게이트② R2 를 worklog `#p0` 에 물려줬다**(「정상 1.1~1.5」가 부하 하 **0.6** 까지 내려온다) — 임계 회차가 그것을 봐야 한다.
  ★**R5(3필드 배선의 단위시험)는 «안 했다»** — 이 티켓 범위가 두 구간 + 순서 잠금이라 남긴다.
  ★★**[2026-09-08 `-fix2`] 그 순서 주석이 «코드가 안 지키는 불변식»을 선언하고 있었다 — 참으로 만들었다.**
  richness 의 **MAX 판본 3필드**(`distinct_colors`·`nondominant_bp`·`center_nonuniform_bp`)가 게이트 «뒤»에 남아 있었는데
  주석은 「every field that merely RECORDS … before the one thing that JUDGES」라고 적었다. ⇒ 그 셋을 게이트 «위»로 올렸다.
  ★**행위 변화 0**: 게이트는 bool 3개의 순수 함수이고 `screen.max_*` 는 paint 에서만 쓰이고 그 세 줄에서만 읽힌다.
  ★**락을 «트리오당 바늘 하나»로 넓혔다** — 하나만 박으면 다른 트리오가 게이트 아래로 흘러도 통과한다(그것이 개악 프로브가 green 이던 구멍이다).
  ★**양방향**: 트리오를 되돌리면 **FAILED**(rc=101) · 새 바늘만 빼면 **통과**(rc=0) ⇒ 바늘이 실효를 갖는다. 전 시험 **168/0**.
- 2026-09-08: **`verify-browser` 가 저장소에 있는데 한 번도 자동 실행되지 않던 문제 — ★⒜(배포 «뒤» 스텝) 채택**
  ...
  ★**URL 은 «별칭»이 아니라 `steps.deploy.outputs.deployment-url`**(액션이 돌아올 때 이미 live) — `wie-web.pages.dev` 별칭의 전환 지연은 **아무도 재지 않았다**.
  ⇒ ★**이 스텝은 «아티팩트»를 검증하고, 게이트③ 4-C 손 실행은 «별칭»을 검증한다 — 서로를 대체하지 않는다.**
  ★★**⑶「red 를 누가 받나」의 답 = «착지시킨 게이트③ 회차»** 이고 ★**`AGENTS.md` 에 적었다**(회신에만 적으면 다음 회차가 모른다).
  ★**hard-fail 로 둔 근거는 폭발반경 실측**: 이 스텝은 push 전용이라 **PR run 은 건너뛴다** ⇒ ★**PR 0건 차단 · 게이트③ 0건 차단**
  (2026-09-07 오전 `check-worklog-coverage` red 가 열린 PR **전건**을 막은 것과 **다른 형태**다) ⇒ `continue-on-error` 가 살 이유가 없다.
...
- 2026-09-07: **핀 이동 시 「경로 밖」 판단이 낡는 문제 — ★⒜(상시 검사) · 단 «스크립트»가 아니라 «워크스페이스 시험»**
  ...
  ★★**[정정] 초판 F1⑷ 는 «거짓»이었다**(게이트② 반증 · 이 회차가 재현): 「lock 재생성마다 움직인다 · 최근 이동은 RUSTSEC 부수효과 ·
  읽어 줄 회차가 없다」 **셋 다 틀렸다**. 다시 잰 값 — lock 줄을 «실제로 바꾼» 커밋 ★**31건 · 2025-07-02~2026-04-11(283일 · 9.4일/회)**
  ⇒ ★**그 뒤 149일째 고정** · 그 RUSTSEC 커밋(`74c924f0`)의 `Cargo.lock` diff 에 wipi 줄 ★**0건** ·
  ★**31건 중 19건이 dependabot 전용 PR**(`dependabot.yml` cargo **daily**) ⇒ ★**읽어 줄 회차는 «있다».**
  ⇒ ★★**⒜를 «다시» 판정했고 유지한다 — 논거를 옮겼다**: ★**빈도에 기대던 초판 논거는 폐기**하고 →
  ★**「⒝·⒞ 는 이제 «가능»하나 19/31 만 덮고 12/31(기능 커밋 부수효과)을 못 본다 · 시험은 31/31 을 덮고(dependabot PR 도 CI 를 탄다)
  비용이 0에 가까워 빈도가 결정에 들어오지 않는다」**. ★⒟도 기각 사유가 바뀌었다 — 「149일 고정」은 오히려 **조용함을 더 위험하게** 만든다.
  ★**제안이 ⒜에 건 세 반박은 «구현»에 대한 것이었다**(`~/.cargo` 를 읽는 스크립트) — 시험 형태면 경로 차이·오프라인·무관 PR 오탐이 전부 비껴간다.
  ★**F1⑵ 의 결정적 관찰**: RustJava 핀 선례는 «사람 약속»이 아니다 — 가리키는 시험이 **무조건 돈다** ⇒ 따라 할 것은 «문장»이 아니라 «시험»이다.
  ★**개악 대조(가짜 핀 이동)**: M1(상류가 `Eof = -23` 추가) → ★**컴파일 에러 `E0004` non-exhaustive** · M2(`-18 → -19`) → ★**시험 FAILED `left: -19`** · 원복 rc=0.
...
- 2026-09-07: **재측 판정이 «마지막에 붙은 행»을 읽던 문제 — ★⒜(max) 채택** (PR **#128** 착지 · `wie-worklog-coverage-last-vs-max-decision`
  ...
  ★**F1⑶ 은 「단조」다**(`[13,23,33,43,53]` · 중복 0 · `at(-1)==max`) ⇒ 소견대로면 ⒞·⒟ 쪽인데 ★**⒜를 골랐다**:
  ★★**역전을 만드는 «전제 조건»이 이미 관측됐다** — `80809604`(#120)와 `ed70b279`(#122)가 **각자 `landedRounds: 53` 을 append** 했고
  ★**바이트 동일이라 한 행으로 접혔을 뿐**이다(값이 하나만 달랐어도 두 행이 남고 순서는 머지가 정한다).
  ★**「게이트 약화가 아님」을 «세 경우 실행»으로 갈랐다**: `[…,50,40]` 역전 **rc=1 → 0**(거짓 red 제거) · `[…,40,50]` 정순 **0 → 0**(불변) ·
  ★**`[…,40,45]` 진짜 기한 초과 rc=1 → 1**(여전히 red). ⇒ 움직인 것은 **한 경우**뿐이다.
  ★**BELOW 축은 오히려 «강해졌다»**: `[…,{56,50%},{50,100%}]` 에서 옛 판본 **rc=0**(임계 미달을 가림) ↔ 새 판본 **rc=1**.
  ★**남는 한계**: 이 script 계열엔 시험 하네스가 없어 세 경우가 «영구 잠금»이 아니다 ⇒ **헤더 주석에 재실행 가능한 형태로** 남겼다.
  ★착지 시 배포 없음 예상(`.rs`·`Cargo.*` **0건** — 착지 diff 로 다시 셀 것).
- 2026-09-07: **`wie_validate` 가 모으던 게스트 stdout — ★«옵트인»으로 싣는다** (PR **#127** 착지 · `wie-validate-drops-collected-guest-stdout`
  ...
  ★착지 시 **실배포 2건** 예상(`web.yml` + `publish-artifact` — 착지 diff 에 `.rs` 가 있다).
- 2026-09-07: **`--expect-last-frame` CI 승격 판정 — ★«올리지 않는다»** (PR **#125** 착지 · `wie-expect-last-frame-ci-promotion-decision`
  · 채택 제안 `2026-09-07-expect-last-frame-wired#p0`) — ★**워크플로 변경 0 · 코드 변경 0**(판단은 `AGENTS.md` 러너 절에).
  ★★**차이축이 «둘»이고 «호스트 축만» 공허하다**(게이트② 반례 · 초판은 델타를 **0** 이라 단정했고 그것은 **거짓 부재 주장**이었다):
  ⑴**호스트 축은 비어 있다** — `HeadlessScreen::paint` 은 프레임 저장 + 카운터뿐인 ★**순수 싱크**(그리기·합성·순서 로직 **0줄**)
    ⇒ 화면을 검게 만드는 원인은 전부 `Screen` 경계 «위» = 두 호스트가 공유하는 엔진에 있다.
  ⑵★**시각축은 «비어 있지 않다»** — `tickLoop` 은 `until` 이 맞는 «순간» 끊는다 ⇒ E+F 는 「어느 틱 경계에서 기대 픽셀 수가 관측됐다」를,
    플래그는 「런의 **최종** paint 가 비-공백이다」를 단언한다. ★**마지막 키**가 match 로 break 한 뒤 F 는 `key_up` → `free()` 라
    그 «이후»를 못 본다(앞 키는 다음 키 단언이 재확인한다). 헤드리스 `--inject` 는 27스텝을 끝까지 돌고 ★**`paints` 55**(양 캐리어 실측).
  ★**5축 대조**: 픽스처 동일 · ★**읽는 프레임이 «다르다»**(E+F = 첫 match 프레임 ↔ 플래그 = 최종 paint) ·
  술어는 ★**E+F 가 «정확한 픽셀 수 · 키별»**(3키×2캐리어 = **6단언**) ↔ 플래그는 «비-공백 boolean» · 호스트 ·
  ★플래그 «자신»의 로직도 이미 CI 커버(`last_frame_gate_fails` **8행 진리표**가 6다리에서 돈다).
  ⇒ ★★**탐지 델타는 «0 이 아니라 좁다»** — 마지막 키 단언 «이후» 구간에 한정되고 ★**그 구간의 실사고 관측이 0건**이다.
  ⇒ ★**그 좁은 델타를 위해 6다리 6회(⒜)나 node 전용 잡의 `cargo` 빌드 신규(⒝)를 «사지 않는다»** — 그것이 ⒞의 논거다.
  ※★2026-09-05 LGT 사고는 이 창을 «가린다» — 같은 틱 «안»의 overpaint 라 캔버스가 424 를 틱 경계에서 한 번도 못 보여 줬고 early-break 가 발동하지 않았다.
  ★**paths-filter 우려를 «수»로 닫았다**: 그 필터 첫 원소가 `**/*.rs` 라 마지막 프레임을 망칠 diff 는 전부 발화한다
  — 주석뿐인 `.rs` 착지 `a4fda020` 에서 「browser boot round-trip」이 **실제로 돌아 success**.
  ★**되돌아올 신호 3가지**를 박았다 — ★**첫 항목이 ⑵의 그 창**이다(「E+F 가 «구조적으로» 못 보는 구간에 회귀가 착지하면」 ·
  ★그것은 **CI 로 오지 않는다** — 로컬 러너나 사용자 보고로 온다). 그리고 셋째에 ★**정직한 한계**를 적었다 —
  출하 네이티브 호스트(`WindowHandle`)는 ★**두 그물 어느 쪽도 덮지 않으며 이 플래그를 승격해도 덮이지 않는다**(그것은 `HeadlessScreen` 을 돈다).
  ★착지 시 배포 없음 예상(`.rs`·`Cargo.*` **0건** — 착지 diff 로 다시 셀 것).
- 2026-09-07: **ABI 어휘 밖 반환값 4건의 처분 — ★«전부 ⒝(그대로 두고 자인)»** (PR **#124** 착지 · `wie-abi-vocabulary-outliers-disposition`
  ...
  ★**요지는 「전부 ⒝」가 아니라 «사유가 건마다 다르다»는 것이다** — 한 규칙으로 접으면 ⑵가 「놓친 것」으로 읽힌다:
  ⑴`socket_close` = **스텁**이라 어떤 특정 변종도 «거짓 주장»(무엇이 실패했는지 스텁은 모른다) ⇒ ⒜에 표적이 없다.
  ⑵`seek_record_single` = ★**어휘가 «정확히» 맞는 유일한 자리**(`Invalid = -9` · 같은 함수가 이미 `-25`)인데도 두었다 —
    직전 회차가 그 대가를 치른 이유는 **UB** 였고 ★**여기엔 UB 가 없다**(사는 것이 «일관성»뿐이면 지불하지 않는다).
  ⑶`stream_read` EOF = ★**어휘에 대응이 아예 없다** · 그 자리 주석이 「실제 타이틀이 EOF 뒤에 부른다」를 기록 ⇒ **가장 위험한 자리**.
  ⑷`clip_put_data` = 게스트 래퍼가 `if result < 0 → MediaError::Platform(i32)` 로 ★**수를 «데이터로» 나른다** ⇒ SDK 계약을 만족한다.
  ★**F1⑵ 재훑기**: `from_raw` 호출 **5곳 = 두 함수뿐**(`get_resource`·`create_image`) ⇒ 4건 전부 경로 «밖» 확인.
  ★**F1⑶ 내부 분기 독자 = 0**(`-1` 비교식 2건은 «입력 센티넬»과 doc 주석 — 무관).
  ★자인은 **네 자리 주석 + 신규 `docs/wipi-c-abi-error-codes.md`** 에 들어갔고, ★**뒤집힐 조건 3가지**도 함께 박았다.
  ★**실배포 «2건»이 맞았다** — 착지 diff 의 `.rs` 3건이 «전부 주석뿐»인데도 `publish-artifact` 가 발화했다
  (`paths-filter` 는 «내용»이 아니라 «경로»를 본다): Pages + 릴리스 `engine-a4fda02` + otterpebble dispatch.
- 2026-09-07: **형제 `-18` 두 자리에 시험** (PR **#123** 착지 · `wie-sibling-shortbuf-sites-have-no-tests`
...
- 2026-09-07: **재측 약속에 «소유자»가 없어 main 이 red 였다** (PR **#122** 착지 · `wie-worklog-remeasure-has-no-owner-and-main-is-red`)
  — ★**F1**: 멱등 `--record` 로 착지 53 기록(2회 실행 · 2회차 **0건 추가** · 삭제 0 = append-only).
  ★**F2 = ⒜(게이트③ 회차가 진다)**. 고른 이유는 「가장 작다」가 아니라 ★**막히는 역할과 고칠 수 있는 역할이 같다**는 것 —
  두 워크플로가 `fetch-depth: 0` 이라 밀린 동안 ★**main 뿐 아니라 열린 PR 전건이 red** 다.
  ★★**규칙만으로는 안 됐을 이유를 찾았다**: 머지 계약의 원장 열거는 `docs/worklog/**` 인데 이 기록 파일은 그 **형제**라
  ★게이트③에 «만질 권한»이 없었다 ⇒ AGENTS.md 원장 목록에 추가(`docs/report/**` 선례와 같은 형태).
  ★**F3 세 겹**: 규칙(AGENTS.md 인용 블록) · 권한(원장 목록) · ★**발화 지점**(검사기 OVERDUE 메시지의 `OWNER:` 절 — 문서가 아니라 «실패하는 순간»에 둔다).
  ★**남는 구멍을 수로**: 주기 10 = **9h 18m** · 착지 간격 중앙 **70.5m** · ★**최대 115.2h** ⇒ 조용한 구간엔 며칠 red 다.
  받아들인 근거 = ★**이 검사는 착지로만 밀리고 착지만 막는다**(조용하면 기다리는 것이 없다). 거짓이 되면 답은 ⒝다.
  ★**비율·임계·판정식·워크플로 무접촉**(변경은 OVERDUE 문자열 한 곳뿐) · 기존 `measurements` 무접촉.
  ★**실배포는 «1건»이었다**(「배포 없음」 예상은 틀렸다 — `web.yml` 은 `paths` 필터가 «없어» 항상 발화한다):
  Pages. `publish-artifact` 는 예상대로 미발화(`.rs`·`Cargo.*` 0건). ★`engine-contract` 가 main 에서 green 이다.
- 2026-09-07: **`--expect-last-frame` 를 «켰다»** (PR **#120** 착지 · `wie-expect-last-frame-gate-has-zero-callers`
...
- 2026-09-07: **`get_resource` 의 버퍼-부족 코드가 ABI 어휘 밖이었다** (PR **#121** 착지 · `wie-get-resource-shortbuf-code-is-not-in-the-abi`
  ...
  ★**실배포는 «2건»이었다**(예측 1건은 틀렸다 — 착지 diff 에 `.rs` 가 있어 `publish-artifact` 도 발화했다):
  Pages + 엔진 아티팩트 릴리스 `engine-a862dfb` + otterpebble `repository_dispatch`. 운영 wasm 해시 변경으로 확인.
...
- **P3**(L·med·★**선행 = P2 아님**. P2 가 답을 못 내므로 P1 결정 뒤로 붙인다) — **여러 회차로 쪼갠다. 첫 조각(개명)은 끝났다.**
  ...
  - ★**부수 발견(총괄 판단 대기)**: 연번 **`0113` 을 열린 PR 둘이 함께 claim** 한다
    (`--next-serial` → `열린 PR claim [0113(#159) 0113(#160)]`). ★착지 전이라 검사기는 아직 `OK` 를 내지만
    **둘 다 착지하면 `main` 이 red** 다. ★`AGENTS.md` 규율은 「착지 안 한 쪽을 옮겨라」인데 ★**둘 다 미착지**다.
    ★**이 회차가 남의 PR 을 고치지 않았다** — 옮기는 쪽은 **0115** 를 잡아야 한다(0114 는 이 회차가 썼다).
