...
## 완료 (최근)
- 2026-09-17: **「이 검사기는 어디서 도나」를 기계가 센다 — 38개 중 호출자 0이 8 · 1이 15** (★2026-09-18 `-fix` 재측 · 초판 「37 중 0이 4 · 결함 0」은 **틀렸다**)
  (`wie-count-checkers-with-only-one-caller` · 채택 `2026-09-17-upstream-guard-wired#p0`) —
  정본 `docs/report/0155--….md`. ★**제품 동작 0줄 · Rust 0줄 · 기존 검사기 배선·삭제 0 · 차단 0.**
  `scripts/checker-census.mjs` 신설 + `engine-contract.yml` **상시 스텝**(★`continue-on-error: true` ·
  ★**러너 실측 0.6초** · 로컬 2.6~2.9초) + `AGENTS.md` 진입점 1문단(★fenced `sh` 아님 ⇒ doc-liveness 복사 의무 불변 · 25줄 그대로).
  ★**수**: base `f7a1d022` 기준 **N=36 · a(호출자 0)=4 · b(호출자 1)=14**(회차 후 37·4·15 — +1 은 도구 자신이고
  ★**버킷 1** 이다 = 새 고아를 만들지 않았다). `scripts/check-*.mjs` **13개 중 단일 호출자 9 · 0 호출자 1 · 2+ 3**
  ⇒ 제안이 스스로 건 임계(「2~3이면 개별 가드가 싸다」)는 **수로는 넘었다**.
  ★★**그런데 그 수로 결정하면 안 된다 — a+b 19행을 전수 분류하니 «고쳐야 할 것» 0건이다.**
  PR 게이트 8건은 호출처가 하나인 것이 **설계**이고, 0 호출자 2건은 둘 다 사유가 적힌 정상이다
  (`check-branch-protection-claim` = Actions 토큰 **403** 이라 못 잰다 · `smoke_gate.sh` = 게임 바이트라
  **Constraint 9 로 구조적 CI 불가**). ⇒ ★**단일 호출자를 red 술어로 쓰면 9건 전부 오탐**이다.
  ★**기존 가드 둘의 실제 술어는 «호출자 수»가 아니다** — 「그 하나를 지워도 green 인가」이고, 그 축은
  `wie-count-deletable-checks-that-stay-green-repo-wide`(PR #94)가 이미 세었다 ⇒ ★**두 수를 «곱해야»
  가드 판단이 된다**(제안 카드 #p0).
  ★**세는 방식 = 배선에서 «역방향»** — 이름 규칙(`check-*`)에 안 기댔다(규칙 «밖» 검사기 실재:
  `audit-no-leak.sh`·`verify-browser.mjs`·`smoke_gate.sh` / 규칙 «안»에 검사기 아닌 것도 실재).
  ★**호출자에서 두 가지를 걸렀다**: ⑴**주석**(`# NOT-RUN:` 은 «안 도는» 명령을 이름으로 적는 줄이다)
  ⑵★**«데이터로서의 경로»**(상수·오류 문구) — 안 걸렀으면 **7행이 틀린 버킷**에 앉는다(실측).
  ★**글롭 호출자는 «누가 부르는지»만 규칙이고 «무엇에 닿는지»는 cargo 에 «묻는다»**: 그 규칙을 빼면 **호출자 0이 8 → 15**
  = 규칙 하나가 38행 중 **7행**을 지탱한다(도구가 이 수를 기본 출력에 **항상** 찍는다).
  ★★**[게이트② 반려 승계 2026-09-18 `-fix`] 초판의 수는 «전부» 틀렸다 — 글롭 커버리지를 «경로 모양»으로 추측했기 때문이다.**
  ⑴`wie_j2me`·`wie_jvm_support`·`wie_midp` 는 **`Cargo.toml` 이 없다**(base swap 이 남긴 고아 · member 는 하이픈 판)인데
  경로 규칙이 그 **4파일에 호출자 4건씩**을 붙여 ★**가장 잘 배선된 «2+» 버킷**에 넣었다 — ★도구 머리주석이 스스로
  「이 census 가 없애려는 바로 그 거짓말」이라 적은 형태를 **도구가 만들었다**.
  ⑵루트가 **자기 패키지**(`[package] name = "wie"`)라 `tests/font.rs` 가 실재 타깃인데 모집단에서 **빠졌다**.
  ⇒ 이제 `covers` 를 ★**`cargo metadata --no-deps` 에 묻는다**(integration-test 타깃 **7건**). 재측:
  base `f7a1d022` **37 · 0=8 · 1=14 · 2+=15** / head **38 · 0=8 · 1=15 · 2+=15**.
  ★**대가**: 계수기가 **cargo 에 의존**한다 ⇒ cargo 부재·offline 이면 ★**`UNMEASURABLE` + rc=2**(실측 확인)이고
  **「0 호출자」로 뭉개지 않는다** — 뭉개면 0버킷이 과대(8 → 15)로 나온다.
  ★★**그리고 「결함 0건」은 «0이어서 0»이 아니라 «셈에서 빠져서 0»이었다** — 새로 드러난 4행이 전부
  ★**아무도 컴파일하지 않는 시험 파일**이다(열린 제안 `2026-09-17-revive-orphaned-preload-guard#p0` 의 「고아 5파일」 잔여 = 정확히 이 4건).
  ★**양방향(끊으면 나타나는가)**: 격리 워크트리에서 배선 3개만 끊자 **1→0 · 2+→1 · 2+→1** 이고 요약이
  `0=8·1=15·2+=15` → ★`0=9·1=16·2+=13` (★2026-09-18 «고친 도구»로 재측 · 커밋 0 · 워크트리 제거 · 본 트리 클린).
  ★**`bin/suite-orphan-check` 차용**: ⒜「판정을 재구현하지 않는다」(배선 목록을 상수로 복제하지 않고 워크플로를
  매번 읽는다) ⒝「못 쟀다 ≠ 깨끗하다」(rc=2 UNMEASURABLE · 못 읽은 표면은 `★UNREAD` 로 「수는 하한」) —
  ★**«3택 면제»는 «일부러» 안 빌렸다**(그것은 게이트를 조용히 시키는 장치이고 이 산출물은 게이트가 아니다 ·
  면제 목록을 두면 분류가 사는 곳이 둘이 된다).
  ★**천장 4**: ⑴워크플로 **인라인** 검사(6종·12호출처)는 파일이 아니라 모집단 밖 — 정의가 곧 호출처라 언제나 1
  ⑵소스 안 `#[cfg(test)]` 는 모듈 단위라 못 센다(**.rs 101파일** · 선행 회차 기록은 31 이었다 — base swap 후 늘었다)
  ⑶cadence 는 워크플로 전체 `on:` 이라 **상한**이다(스텝 `if:`·paths 스코프 미해석)
  ⑷`uses:`/`with:` 액션 입력은 «실행»이 아니라 `named-not-run` 열로 낸다(재사용 워크플로·composite action **0건**).
