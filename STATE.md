...
## 완료 (최근)
- 2026-09-17: **「keydraw_ktf 가 부하에 더 약하다」를 재서 «반증했다» — 짝지어 재니 5/18 대 5/18**
  (`wie-adopt-keydraw-ktf-load-fragility-cause` · 채택 `2026-09-17-adopt-slice-d-base-swap-executed-p1-p0-p0#p0`) —
  정본 `docs/report/0154--….md`. ★**코드 0줄**(조사 회차 · 시험 임계·재시도 무접촉).
  제안 근거는 **ktf 4/6 ↔ lgt 0/6** 이었는데 그 관측은 **짝지어진 것이 아니었다**. 두 픽스처를 한 루프에서
  **교대**로 돌려 같은 부하 분을 보게 하니 ★**ktf 5/18 · lgt 5/18 로 동일**(0/6·0/6 → 2/6·3/6 → 3/6·2/6).
  ★그 크기의 캐리어 고유 효과라면 18회에서 **약 12 대 약 0** 이었어야 한다 ⇒ **그 크기는 배제**(작은 차이는 이 n 으로 못 배제).
  ★**실패 모드 전건 동일**: 10건 모두 `only blank/uniform` · paints **11~22**(건강할 때 55) · 패닉·tick 오류 0.
  ★**설명 = 짝짓지 않은 표집** — 배경 부하가 통제 변수보다 크게 움직인다(내 실험 안에서도 `+0` 구간 loadavg **74.8** ↔ `+12` 구간 **50.1** 로 역전).
  ★**제안이 지목한 두 축은 둘 다 갈림점이 아니었다**: 부팅 비용은 `--boot-secs 0.3` 에서도 양쪽 0/3 ·
  ★**`ticks` 는 못 쓴다**(동일 조건 lgt 5런 = 9,630,471 / 912,303 / **36** / 103,786 / **35** — 블록 중 «스핀»을 센다.
  ★이 회차가 그 값으로 「4.2배」 결론을 낼 뻔했고 그것은 틀렸다) · `--action-secs` 노브는 실부하의 대역이 **아니다**.
  ★★**[게이트② 반려 승계 2026-09-18 `-fix`] 그 «이유»가 틀렸었다** — 초판은 「0.05 에서 **부팅을 깨며**」라고 적었는데
  ★**부팅은 원리적으로 모자랄 수 없다**: `deadline = min(boot+0.3+27×action+1.0, 120)` 이라 **deadline−부팅예산 = 27×action+1.0** 이고
  `boot_secs` 가 **소거**된다(0.05 에서 여유 **+2.35s** · A(2.5)와 B(0.3)가 **정확히 같다**).
  ★★**그리고 실측은 그것을 «증명하지 못한다» — 그 사실도 함께 적었다**: 2×2 요인실험이 저부하(13~95)에서 **48/48 PASS**(공허) ·
  고부하(105~148)에서 A 3/6 · B 4/6 · C 1/6 · ★**D(=기본) 2/6** 으로 **기본 설정조차 무너져 잡음이 판정을 덮는다**
  (부팅만 다른 쌍의 불일치 A↔B 3/6 · C↔D 1/6 인데 **방향이 반대** ⇒ 잡음). ⇒ ★**결정근거는 산술 하나이고, 이 실험을
