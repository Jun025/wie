...
## 완료 (최근)
- 2026-09-17: **제안 #p0 은 «참»인데 «이 보드의 일이 아니다» — 카드 스키마는 orchestrator 소유다**
  (`wie-adopt-corpus-card-first-question-p0` · 제안 `2026-09-16-corpus-card-first-question#p0`) —
  정본 `docs/report/0137--….md`. ★**wie 제품 코드 0줄 · `~/orchestrator` 파일 «0줄»**.
  ★**전제는 재측으로 여전히 참**: `grep -c 'body' bin/humanstep-scan` **0** · 카드 **99장** 중
  본문 3줄 이상 **57장** · 본문 합 **2,269줄**(제안 기재 2,254 대비 **+15** — 그 사이 새 서사) ·
  스키마의 해당 줄은 `(본문은 선택 — 배경·주의사항)` 그대로.
  ★**집행하지 않은 근거 4축**: ⑴제안 스스로 `target: orchestrator` ⑵wie 트리의 `humanstep` 참조
  **4건 전부 원장**(제품·scripts·CI **0**) ⑶그 파일은 estate 3곳 **99장**을 지배하는 «전역 계약»
  ⑷★**결정적 — orchestrator-ops 에 «untracked»**(`humansteps/` 101파일 중 tracked **5**) ⇒
  거기 손대면 **PR·게이트②·revert 경로가 전부 없다**(형식은 wie 회차, 실질은 3게이트 밖의 전역 계약 변경).
  ★**wie 안에 사본을 두는 우회로는 wie 자신이 금지한다** — `AGENTS.md:39` 「외부 계약은 참조하되 복사하지 않는다」.
  ★**형제 회차와 갈리는 지점**: 직전 `…-triage-r2-p0` 이 고친 것은 **자기 티켓을 막던 카드 한 장(leaf)** 이고,
  이번 것은 **그 카드들을 규정하는 계약**이다.
  ★**산출물** = 재측표 + ★**그 자리에 그대로 붙일 문안 초안**(report 0137 전문) + 라우팅 결함 1건:
  제안의 `target` 을 주입 지시문이 **찍지 않고** `repo` 를 **worklog 위치**로 정한다
  (`cockpitd.js:2727·:3095` 는 payload 에 싣는데도) ⇒ 후속 제안.
  ★**처분은 `declinedProposals`** — 「제안이 틀렸다」가 아니라 「이 보드가 집행할 수 없다」다.
  이 ref 를 처분한 worklog 가 **0건**이라 안 적으면 패널이 **같은 제안을 wie 로 다시** 내민다.
  ★**해소 조건**: 총괄이 `orch-*` 로 재발권(정본 문안은 0137 §산출물).
