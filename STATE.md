...
- 2026-09-18: **`game_lab/broken` 187 census 를 «오늘 트리»로 다시 떴다 — 40%가 서명을 바꿨고 1위 군집이 «새것»이다**
  ...
  ★★**분류 규칙이 «어디에도 기록돼 있지 않아» 역산해 복원했다** — 7월 공표표에 적용해 **37·37·15·6·4·87 = 186 «6/6 정확 일치»** 를
  확인한 뒤 «같은 규칙»으로 오늘을 채굴했다(급소 = `Method …@N not found` 16 · `Unknown lgt stdlib import` 6 이 `Fatal error` 안에 숨어 있었다).
  ★**두 표(FAIL 만)**: `NoSuchMethod` **37→8** · `panic-unwrap` **37→15** · `unimpl-stub` **15→6** · `unknown-stdlib` **6→2** ·
  `panic-unreachable` **4→0** · `UNCLASSIFIED` 80→99 · **FAIL 179→130 · PASS 7→57** · ★**186 중 75건(40%) 버킷 변경**.
  ★★**오늘 1위는 `no frame rendered` 32**(7월 **1**) — ★**부하 인공물이 아니다**: 20초 예산을 다 태우고(`ms` 중앙 21,232)
  `paints` **0**, 재실행 틱 **4,875만·2,901만**. ⇒ **다음 수리 캠페인의 1순위**.
  ★**혼입 변인을 숨기지 않는다**: `PASS 7→57` 의 상당 부분은 «다른 JVM»이 아니라 **같은 날 착지한 폰트 수정**이고 **둘을 가르지 않았다**.
  ★**비용**: 전수 **45분** · 그동안 **CPU idle 0.0%** 로 열린 PR 5건이 self-hosted 러너를 기다렸다(병렬화는 verdict 뒤집힘 때문에 안 했다).
- 2026-09-17: **헤드리스 검증기가 «글자를 그리는 순간» 스스로 패닉했다 — `broken/` «표본» 최대 서명이 게임 버그가 아니었다**
  ...
  ★★**그 표는 «2026-07 기준»이다 — 오늘 표는 `docs/report/0165--….md` 다**(2026-09-18 전수 재census: `NoSuchMethod` **37 → 8** ·
  1위가 **`no frame rendered` 32**(7월 1)로 **바뀌었다**). ★이 항목은 **고치지 않는다** — 날짜 박힌 회차 기록이다.
  ★**한계**: 기존 `reports/` 1,312건은 `rustjava rev c66f08d` 시절 것이라 **오늘 트리를 설명하지 않는다**(base swap 후 crates.io `^0.1.1`) — 전수 재census 는 안 했다.
  ※★**그 「1,312」은 오늘 트리에서 재현되지 않는다**(0165 실측: `reports/` json **452** · log **470** · 어떤 조합도 1,312 아님).
