...
- 2026-09-17: **연번 충돌이 «착지 후»에만 울었다 — 열린 PR 끼리도 울게 했다**
  ...
  **커밋→PR 간격**인데 PR **32건** 실측 **p50 28초 · max 70초**다. 실제 충돌은 **5분·8분** 간격 ⇒ ★**넓혀도 못 막는다.**
  ...
  ⇒ 기본 모드가 `crossPrCollisions` 로 **exit 1** 을 내고 ★**«누가 옮기는가»까지 찍는다**(PR 번호가 큰 쪽 ·
  내 쪽이 PR 이 아니면 내가). ★**오탐을 막는 자리**: 대조 대상은 «디스크 전체»가 아니라 **디스크 − `origin/main`** 이다
  ...
  ★**대가**: PR 빌드가 네트워크를 친다(실패 = 경고 · red 아님) · 충돌 시 **두 PR 이 다 red** ·
