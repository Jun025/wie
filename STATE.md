...
## 완료 (최근)
- 2026-09-17: **「이 검사기는 어디서 도나」를 기계가 센다 — 37개 중 호출자 0이 4 · 1이 15, 그리고 ★결함은 «0»**
  ...
  **Constraint 9 로 구조적 CI 불가**). ⇒ ★**단일 호출자를 red 술어로 쓰면 9건 전부 오탐**이다.
  ...
  ★**글롭 호출자는 규칙으로 «선언»**: `cargo test --all`/`tarpaulin --workspace` 를 빼면 **호출자 0이 4 → 14**
  = 규칙 하나가 36행 중 **10행**을 지탱한다(도구가 이 수를 기본 출력에 **항상** 찍는다).
  ★**양방향(끊으면 나타나는가)**: 격리 워크트리에서 배선 3개만 끊자 **1→0 · 2+→1 · 2+→1** 이고 요약이
  `0=4·1=15·2+=18` → ★`0=5·1=16·2+=16` (커밋 0 · 워크트리 제거 · 본 트리 클린).
