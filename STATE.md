...
- **`--expect-last-frame` CI 승격 판정 — ★«올리지 않는다»** (PR 개설 · `wie-expect-last-frame-ci-promotion-decision`
  ...
  ★**F1⑶ 의 답이 «없다»이고, 그것을 «부재의 부재»가 아니라 «구조»로 닫았다**: `HeadlessScreen::paint` 은
  프레임 저장 + 카운터뿐인 ★**순수 싱크**(그리기·합성·순서 로직 **0줄**) ⇒ 마지막 프레임을 검게 만드는 원인은 전부
  `Screen` 경계 «위» = 두 호스트가 공유하는 엔진에 있고, 그것을 **Scenario E+F** 가 더 강한 술어로 이미 단언한다.
  ★**5축 대조**: 픽스처 동일 · 프레임 동일(둘 다 마지막) · 술어는 ★**E+F 가 «정확한 픽셀 수 · 키별»**(3키×2캐리어 = **6단언**) ↔ 플래그는 «비-공백 boolean» ·
  호스트만 다르고 그 예외가 비어 있다 · ★플래그 «자신»의 로직도 이미 CI 커버(`last_frame_gate_fails` **8행 진리표**가 6다리에서 돈다).
  ⇒ ★**승격은 탐지력을 «0» 만큼 늘리고 대가만 남긴다**(⒜ = 6다리 6회 · ⒝ = node 전용 잡에 `cargo` 빌드 신규).
  ...
  ★**되돌아올 신호 3가지**를 박았고 그중 하나에 ★**정직한 한계**를 적었다 — 출하 네이티브 호스트(`WindowHandle`)는
  ★**두 그물 어느 쪽도 덮지 않으며 이 플래그를 승격해도 덮이지 않는다**(그것은 `HeadlessScreen` 을 돈다).
