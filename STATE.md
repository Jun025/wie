...
- 2026-09-18: **`merge=union` 은 «기각» — 서버가 그 속성을 듣지 않는다. 구조 처방을 막던 백필은 «3건»으로 끝냈다**
  ...
  ★**열린 PR 9건 전수**(04:17:52 · ★**pull ref 를 «같은 턴»에 재fetch**): 충돌 **5/9** · 충돌 파일이 **`STATE.md` 하나**이고
  마커 **5/5 가 63행**(= `§완료` 삽입점). ★**rc=0 4건(#194·#195·#198·#199)의 기전은 «새로 열렸다»가 아니라
  `merge-base == origin/main`(`behind=0`)** 이다 — 그 넷은 §완료에 `+20`·`+45`·`+21`·`+12`행을 넣고도 깨끗하다
  ⇒ ★**main 에 §완료 항목이 «하나만» 더 착지하면 `behind≥1` 전건이 즉시 재충돌한다**(진단이 강화된다).
  ★★**싼 대안을 «실제로 설정해» 판정했다**: ⒜워크트리 `git merge` 는 **듣는다**(가짜 형제 착지 후 **rc=0** · 양쪽 항목 보존)
  ⒞**GitHub 서버 머지 HTTP 409** — ★**merge-base 가 속성을 갖고 양쪽이 상속한 «착지 후» 형상에서** 재측했다
  ⇒ ★**증상(`mergeable: CONFLICTING`)에 닿지 못한다 ⇒ 기각**. ★그 쌍을 **워킹트리에 속성을 둔 `merge-tree`** 로 재면 **rc=0**
  ⇒ ★**git 은 합칠 수 «있는데» 서버가 그 능력을 안 쓴다**는 것이 결정적 다리다.
  ★★**[승계 `-fix` 정정] 초판이 근거로 든 ⒝(「양쪽 브랜치가 속성 보유인데 `merge-tree` rc=1」)는 «측정 착오»라 들어냈다** —
  `merge-tree` 는 속성을 **워킹트리**(또는 `--attr-source`)에서 읽는다. 실측: **untracked** `.gitattributes` 한 장을 두면
  **양쪽 커밋에 없는데도 rc 가 1 → 0 → (지우면) 1** 로 뒤집힌다. ⇒ ★**그 rc 는 union 의 유효성을 말하지 못한다.**
  ★남는 한계: 잰 것은 `POST /merges` 이고 `mergeable` 필드 자체는 아니다(같은 서버 머지라 직접 추론이나 **측정은 아니다**).
  ...
  **도입 병합 자체는 여전히 충돌**한다. ⇒ ★「잃는 것이 없다」가 아니다.
  ...
  ★**이 PR 도 `STATE.md` 를 만진다** ⇒ 착지하면 `behind ≥ 1` 인 나머지 열린 PR 전건을 무효화한다 — **C 전까지 누구도 못 피한다**는 실증이다.
...
- 2026-09-17: **「이 검사기는 어디서 도나」를 기계가 센다 — 38개 중 호출자 0이 7 · 1이 15** (★2026-09-18 `-fix` 재측 · 초판 「37 중 0이 4 · 결함 0」은 **틀렸다**)
  ...
  ★**러너 실측 0.6~2.0초** — ★**같은 코드 3 run 이 2.04 · 1.49 · 0.64 로 갈렸다**(부하) ·
  ★초판의 「0.6초」는 «cargo 를 부르기 전» 판본이나 ★**그 값이 이 범위 안이라 러너 수로는 두 판본을 «가르지 못한다»** —
  ★**배수를 인용하지 마라**)
  + `AGENTS.md` 진입점 1문단(★fenced `sh` 아님 ⇒ doc-liveness 복사 의무 불변 · 25줄 그대로).
  > ★★**[초판값 · 철회됨 — 바로 아래 두 문단은 이 회차가 «스스로 철회한» 수다.** 글롭 커버리지를 경로 모양으로
  > 추측한 판본의 값이라 **N·a·b 가 전부 틀렸다**. 참값과 사유는 이 항목 아래쪽
  > 「**[게이트② 반려 승계 2026-09-18 `-fix`] 초판의 수는 «전부» 틀렸다**」 절 — ★**그쪽을 먼저 읽어라.**
  > (★줄 수로 가리키지 않는다 — §완료는 머리 삽입이라 줄번호가 매 회차 밀린다.)
  > 원장이라 지우지 않고 표지만 단다.]
  ...
  **Constraint 9 로 구조적 CI 불가**). ⇒ ★**단일 호출자를 red 술어로 쓰면 9건 전부 오탐**이다.
  ...
  ★**글롭 호출자는 «누가 부르는지»만 규칙이고 «무엇에 닿는지»는 cargo 에 «묻는다»**: 그 규칙을 빼면 **호출자 0이 7 → 15**
  ...
  ⇒ 이제 `covers` 를 ★**`cargo metadata --no-deps` 에 묻는다**(integration-test 타깃 = ★**현 트리·head 8건**
  · base `f7a1d022` 에서는 **7건**이었다 — ★**닻 없이 인용하지 마라. 이 수도 상수가 아니다**). 재측:
  base `f7a1d022` **37 · 0=8 · 1=14 · 2+=15** / ★**현 base(`origin/main`) 37 · 0=7 · 1=14 · 2+=16** / head **38 · 0=7 · 1=15 · 2+=16**
  (★0버킷이 8 → 7 로 준 것은 형제 회차 `wie-adopt-orphaned-files-after-base-swap-census`(#193 · `docs/report/0153`)가
  고아 `wie_j2me/tests/test_boot.rs` 를 하이픈 member 로 되살렸기 때문이다 — ★**이 수는 상수가 아니라 살아 움직인다**).
  ★**대가**: 계수기가 **cargo 에 의존**한다 ⇒ cargo 부재·offline 이면 ★**`UNMEASURABLE` + rc=2**(실측 확인)이고
  **「0 호출자」로 뭉개지 않는다** — 뭉개면 0버킷이 과대(★head 에서 **7 → 15**)로 나온다
  (★「8 → 15」는 base `f7a1d022` 값이다).
  ★★**그리고 「결함 0건」은 «0이어서 0»이 아니라 «셈에서 빠져서 0»이었다** — 새로 드러난 4행이 전부
  ★**아무도 컴파일하지 않는 시험 파일**이다. ★**그 처분은 이미 형제 회차가 내렸다** — `#193`/`docs/report/0153` 이 고아 **4건**을 전수로 세고 1건(`test_boot`)을
  되살렸으며 **나머지 3건은 되살리면 red**(`E0061` 등)임을 실측했다 ⇒ ★**내 census 의 0버킷 잔여 3건과 정확히 일치**한다.
  ★**양방향(끊으면 나타나는가)**: 격리 워크트리에서 배선 3개만 끊자 **1→0 · 2+→1 · 2+→1** 이고 요약이
  `0=7·1=15·2+=16` → ★`0=8·1=16·2+=14` (★2026-09-18 «고친 도구»로 재측 · 커밋 0 · 워크트리 제거 · 본 트리 클린).
