...
- **핀 이동 시 「경로 밖」 판단이 낡는 문제 — ★⒜(상시 검사) · 단 «스크립트»가 아니라 «워크스페이스 시험»**
  ...
  ★**F1⑷ 가 ⒝·⒞·⒟ 를 한꺼번에 죽였다**: `wipi_types` 는 `Cargo.toml` 에 ★**`rev` 핀이 «없다»**(RustJava 는 있다) ⇒
  락 재생성의 부수효과로 움직인다 — ★**고유 rev 31개 · 2025-07-03~2026-09-06** · 최근 이동은 **무관한 RUSTSEC 범프**와 같은 구간.
  ⇒ ★**읽어 줄 「핀 범프 회차」가 존재하지 않아** ⒝(체크리스트)·⒞(전용 스크립트)는 부를 사람이 없고, ⒟는 전제(「거의 안 움직인다」)가 거짓이다.
