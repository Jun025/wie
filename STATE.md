...
## 완료 (최근)
- 2026-09-17: **「본문에만 사는 human-step 이 몇 건인가」 — «모른다»에서 199건으로 바꿨다(정밀도 53%)**
  (`wie-adopt-slice-d-base-swap-fix3-p1-p0-p0` · 채택 `2026-09-17-adopt-slice-d-base-swap-fix3-p1-p0#p0`) —
  정본 `docs/report/0150--….md` · ★**전수 목록의 정본은 회신**(`~/orchestrator/reports/…-p0-p0.done.md` · 소관이 그쪽이라 여기 복사하지 않는다).
  ★**코드 0줄 · 카드 0장**(계약 「세기까지 · 발견분을 고치지 마라」).
  ★**답**: `reports/*.done.md` **7,198건** 중 「선언했는데 `humansteps/` 카드 없음」 = **199건** · 표본 15건 손 검증 **8/15 = 53%** ⇒ 실 선언 **≈105건**.
  ★★**술어를 두 번 고쳤고 그 과정이 실익이다** — 제안 형태 그대로의 초판은 **576건 · 13%** 였고, ★표본을 «손으로 읽어»
  결함 둘을 잡았다: ⒜`★` 미제거로 「★**없다.**」가 항목으로 셌다 ⒝콜론 뒤 부정형 미검사로 「`human-step: none`」이 히트였다.
  ★★**대조군이 실제로 값을 했다** — 제안의 «그 건»(`wie-main-branch-protection`)은 `## human-step` 절이 **없고 §C 산문**이라
  절-only 술어는 **놓쳤다** ⇒ 산문 축을 더했다. **대조군이 없었으면 그 사례 자신을 못 담는 답을 냈을 것이다.**
  ★★**반증축에서 더 큰 것이 나왔다** — 같은 일을 하는 `bin/humanstep-harvest` 가 **이미 있고** launchd(09:10)로 **매일 돈다**.
  ★그런데 그 경로가 쓰는 **`--carry-json`** 은 `carry`·`resolved` 만 출력한다(`:521-540`) ⇒ ★**미등재 집합이 스케줄 경로에 «아예 없다»**
  — 이 조사가 한 번도 생산되지 않은 **구조적 이유**다(`healthcheck`·`dispatcher` 의 언급은 **전건 주석**).
  ★**남긴 한계**: ⑴199는 «선언» 수이지 «미해소» 수가 아니다(건별 사람 판단이 남았고 **제안이 그것을 실제 비용으로 미리 적었다**)
  ⑵53%는 표본 15건 기준(95% CI 대략 27~79%) — 「105건」을 점추정 이상으로 읽지 마라
  ⑶★**AMBIG 931건은 세지 않았다**(「없음, 단 …」) — **다음으로 큰 미측정 구역이고 199보다 크다**.
