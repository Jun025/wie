...
- 2026-09-18: **`game_lab/broken` 187 census 를 «오늘 트리»로 다시 떴다 — 40%가 서명을 바꿨다(1위 군집의 «새것» 주장은 철회)**
  ...
  ★★**[게이트② 반려 승계 · `-fix`] 표의 «수»는 살았고 그 수에 붙었던 «주장»이 틀렸다 — 넷을 철회한다.**
  ⑴「분류 규칙이 어디에도 기록돼 있지 않았다」는 **거짓**이다 — 7월 회차가 **자기 원장 done 에 `RULES` 표를 공표**해 두었고,
  초판은 「선행 리포트」를 **repo `docs/report/`** 로만 읽어 **repo 밖 원장**을 못 봤다. ⇒ 역산은 «최초 기록»이 아니라 **독립 교차검증**이다.
  ⑵「7월 공표표와 **6/6 정확 일치**」 → **공표표는 15버킷**이다. 정직한 문면은 「**5버킷 정확 재현 · 10버킷을 UNCLASSIFIED 로 접었다(합 87)**」이고,
  ★**7월의 UNCLASSIFIED 는 11** 이다(80·87 은 «접은 잔여»). ★「합 186」은 검증력이 0 이다 — 186 의 어떤 분할도 186 이다.
  ⑶「**같은 규칙**」 → **형식 무관 술어를 새로 만들어 두 입력에 동일 적용**했다(7월 규칙은 슬래시형 앵커 · 오늘 출력은 점형).
  ★★**양방향 실측이 40% 의 기전을 확정한다** — 같은 186 문자열을 형식만 바꾸면 7월 규칙은 **36건(19.4%)** 을 잃고
  (`NoSuchMethod` 37→16 · `unimpl-stub` 15→0), 이 회차 술어는 **0건** 움직인다. ⇒ 40% 는 **형식 인공물이 아니다**.
  ⑷★★**「`no frame rendered` 32 는 부하 인공물이 아니다」를 철회한다 — 가르지 못했다.** 판별식의 `ms` 축은 **벽시계라 판별력 0** 이고,
  그 버킷은 `wie_validate.rs:795` 의 **최종 `else` catch-all** 이며, 게이트② 검수가 **고부하 2/2 FAIL ↔ 저부하 3/3 PASS** 로 인공물을 재현했다.
  ⇒ **「32」는 관측치로 남기고 «1순위» 주장은 내렸다.** ★저부하 재측은 **불가능했다** — ★**per-game 원자료를 남기지 않아 그 32건이 어느 게임인지 모른다**(최대의 대가).
  ★**중복 3 = `놈3`·`다크슬레이어2`·`이노티아연대기2` · 결손 1 = `broken/lgt/크로이센.zip`**(이름을 적었다 — 파일명 4개가 repo 에 들어온 대가를 졌다).
...
- 2026-09-17: **헤드리스 검증기가 «글자를 그리는 순간» 스스로 패닉했다 — `broken/` «표본» 최대 서명이 게임 버그가 아니었다**
  ...
  1위가 **`no frame rendered` 32**(7월 1)로 **바뀌었다** — ★단 그 32 는 **부하 인공물과 가르지 못한 관측치**다). ★이 항목은 **고치지 않는다** — 날짜 박힌 회차 기록이다.
