...
## 완료 (최근)
- 2026-09-18: **`verify_*.png` 파일명 계약 — 검사를 «만들지 않기로» 했다(측정으로 기각 · 코드 0줄)**
  (`wie-verify-screenshot-filename-contract-unchecked` · 채택 `2026-09-17-verify-browser-screenshot-side-effect#p0`) —
  정본 `docs/report/0156--….md`. ★**검사기 0개 · 코드 0줄 · 제품 스크립트 무접촉** — 산출물은 «측정과 결정»이다.
  ★★**제안 자신이 걸어 둔 판정식을 그대로 돌렸다** — 「채택 전에 «이 이름이 움직인 적이 있는가»를 세라 · 0이면 만들지 마라」 ⇒ **0**.
  ★실측 셋: ⑴**대조 검사 0건**(`git grep -ln 'verify_' scripts/check-*.mjs` 빈 출력 · 이름은 생성 1·문서 1·`.gitignore` «패턴» 1 세 곳뿐)
  ⑵★**기계 소비자 0** — 이름을 «읽는» 추적 파일이 `AGENTS.md`·`verify-browser.mjs` 둘뿐이고 `web.yml` 의 `upload-artifact` 는 `web/dist` 만 올린다
  ⇒ 소비자는 «사람»뿐, 낡음의 피해는 «잠깐의 혼동» ⑶**생성 두 줄을 건드린 커밋 1건**(2026-06-24 파일 생성 그 자체 · 이후 같은 파일 커밋 2건은 그 줄 무접촉).
  ★★**반대 증거도 적었다 — 기각을 그 한계 «위»에 세웠다**: 문서 쪽은 **2026-09-17 에 생겨 하루**다
  ⇒ 측정된 것은 「**코드**가 3개월 안 움직였다」이지 「이 **쌍**이 안 갈렸다」가 **아니다**.
  ★**한 벌로 만들기 3안을 각각 기각**(문서 생성 = 산문을 손으로 못 고치게 됨 · 코드가 마크다운 파싱 · 공유 상수는 사본이 안 준다)
  ⇒ 남는 검사는 ★**`<label>` 파생 규칙의 «셋째 사본»**이 되어 **사본으로 사본 드리프트를 막는 자기모순**이다.
  ★**재개 트리거를 측정 가능하게 남겼다**: ⒤생성 두 줄이 바뀌는데 `AGENTS.md` 가 같은 PR 에서 안 바뀐다 ⒥이름을 «읽는» 소비자가 1개라도 생긴다.
  ★**그날의 가장 싼 구현도 미리 적었다** — `doc-liveness.yml` 주간 잡이 `npm run verify` 를 **이미 돌려 그 두 파일을 실제로 만든다** ⇒
  「문서가 적은 **그 정확한 두 이름**이 실재하는가」를 붙이면 규칙 재기술 없이 **실제 코드 경로**를 판정한다(존재 검사가 아니다 · 천장 = 기본 실행 1개·지연 최대 1주).
