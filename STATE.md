...
## 완료 (최근)
- 2026-09-18: **`merge=union` 은 «기각» — 서버가 그 속성을 듣지 않는다. 구조 처방을 막던 백필은 «3건»으로 끝냈다**
  (PR 개설 · `wie-state-md-insertion-point-conflicts-every-open-pr`) — 정본 `docs/report/0162--….md`.
  ★**열린 PR 8건 전수** `merge-tree`: 충돌 파일이 **`STATE.md` 하나**이고 마커가 **7/7 63행**(= `§완료` 삽입점) · **#198 만 rc=0**.
  ★★**싼 대안을 «실제로 설정해» 판정했다**: ⒜워크트리 `git merge` 는 **듣는다**(가짜 형제 착지 후 **rc=0** · 양쪽 항목 보존)
  ⒝`merge-tree` **rc=1** ⒞GitHub 서버 머지 **HTTP 409** ⇒ ★**증상(`mergeable: CONFLICTING`)에 닿지 못한다 ⇒ 기각**.
  ★대가도 쟀다 — 같은 줄을 양방이 고치면 **모순된 두 줄이 조용히**(rc=0·경고 0) 남고, 순서가 **시간순이 아니며**(ours 먼저),
  **도입 병합 자체는 여전히 충돌**한다. ⇒ ★「잃는 것이 없다」가 아니다.
  ★**백필 대상은 「7」도 #140 의 「5」도 아니라 «3»**(`0159`·`0160`·`0161` 신설 · 소실 0) — 세 번째 같은 오분류이고
  이번 축은 **«한 §완료 줄이 네 회차를 묶는다»**(2026-07-22 = PR #36·#39·#42·#43). ⇒ §완료에만 있는 항목 **3 → 0**.
  ★**재생하지 않는다**: 09-08 이후 69건 중 사본 없음 2건 = 그 동시비행분뿐 ⇒ **09-09 이후 67/67 보유**.
  ★**삽입점 제거는 «하지 않았다»**(계약 3 · 별 회차 · 착지 순서는 총괄) · 제품 코드 **0줄** · 열린 PR **무접촉**.
  ★**이 PR 도 `STATE.md` 를 만진다** ⇒ 착지하면 나머지 7건을 무효화한다 — **C 전까지 누구도 못 피한다**는 실증이다.
