...
> ★★**[benefit 절반 측정 2026-09-24 · `…-lgt-corpus-decides-graphics-wiring-adopt-p2`(+`-fix`) · 정본 `docs/report/0236--….md`]
> 위 「남은 절반」을 쟀다 — ★benefit 은 «1» 이다.** `broken/lgt` 46건(= 서로 다른 37건 + sha256 동일 사본 9건 ·
> 사본은 2026-09-24 `_dup/` 로 이동) × 두 배선을 짝지어 돌렸다. 15초 패스는 차이 0 이었으나 ★**15초 데드라인이 판정축을 가렸다** —
> «no frame»·«blank» FAIL/FAIL 9건을 `--timeout 40` 짝지은 2쌍(load1 < 40)으로 재측하자 **1건이 ⒜ PASS ↔ ⒝ 단색(흰 화면)** 으로
> 뒤집혔다(순서 교대 2/2 · 게이트② 독립 재측 4/4). ⒜ 는 이용안내 글자를 그리고 ⒝ 는 아무것도 못 그린다. 나머지: 양쪽 PASS 1 ·
> 양쪽 단색 2 · ★**판정 불가 5**(양쪽 페인트 전 정지).
> ⇒ ★**저울 = benefit 1 ↔ ⒜ 의 `keydraw_lgt`(Scenario F · required check) 파손. 결정 ⒝ 유지** — 근거는 「benefit 0」이 아니라
> 「⒜ 는 required check 를 깨는 전면 교체이고, 그것이 사는 1건은 ⒝ 의 텍스트/그리기 결손 하나라 공용 쪽에서 메우는 편이 좁다」.
> ★**한계**: FAIL 28 중 19건은 부팅 중 오류로 graphics 에 닿기 전일 수 있다 — `0230` 이 짚은 «#161 이 떨군 LGT 행»이
> 복원되면 그 일부가 graphics 에 닿으므로, **그때 이 질문을 한 번 더 물을 값이 있다.** 판정 불가 5건도 같다.
