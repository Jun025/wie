...
>
> All three of them are named in this section's own buckets, and both bucket *labels* are now wrong:
...
> `놈3` is **splash only and shallow** — a logo plus a green sprite, no menu inside the budget).
...
| **per-frame render driver** | ★**SUPERSEDED 2026-09-21** — was: ⛔ blocked on ez-i render-tick ABI (§7), **0 draw calls** (AOT-Java path). A `RUST_LOG=debug` trace of `메이플스토리2007` shows the loop running: `net.wie.EventQueue::getNextEvent` 73×, `dispatchEvent` 73×, `net.wie.CardCanvas::paint` 21×, `Graphics::drawImage` **193×**, `Display::pushCard` 1×. ★**Those counts are load-dependent — do not re-cite them as absolutes**: one run (release `394fde8b`, `--inject --action-secs 0.3`, loadavg 47–130); a gate② re-trace of the same binary read `184×`/`184×`/`130×`/`3,584×`, **up to 18× higher**, because the `--inject` deadline is a fixed wall-clock budget. The load-invariant claim is only that each is **non-zero**. Driven by `RepaintEvent(41)`; `TIMER_EVENT` appears **0×** in the tree. ★**Not a claim that §7 is solved for the other 15** — they die at boot, upstream of it, so nothing reaches §7 to test |
