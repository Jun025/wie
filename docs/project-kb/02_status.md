...
>
> ★★**[2026-09-08] 그 문장을 이제 «기계가» 읽는다** — `scripts/check-audit-warnings.mjs` 가 같은
> `cargo audit` 출력으로 **§A 행 집합 ↔ 경고 집합**을 대조하고, 어긋나면 매일 도는 Security audit 잡이
> **red** 가 된다(`wie-supply-chain-ledger-staleness-invariant-in-checker`).
> ★**«수»가 아니라 «집합»이다** — 2026-09-06 사고는 **계수가 같고 구성이 달랐고**, 계수 비교는 그것을 못 본다.
> ★**양방향 + 절 머리 선언 수**까지 본다. ★**표를 못 읽으면 «조용히 통과»가 아니라 «못 쟀다»로 운다.**
>
> ★**세는 대상의 정의**: 줄머리가 `| **A-<n>** |` 인 **A 표 행만**. C 로 옮긴 해소분은
> `| **C-2**<br>(구 **A-2**)` 로 시작하므로 **세지 않는다**(구 ID 병기는 셀 «안»에 있다) —
> ★**C 는 「이미 해소된 것의 노출 판정 보존」이고 그 절이 스스로 이 불변식 «밖»이라고 적는다.**
> ⇒ ★**행을 지우지 말고 C 로 «옮겨라»** — 그것이 이 검사를 통과시키는 유일한 정상 경로다.

### A. 권고·공급망 2건 — **전건 비게이팅 경고**(`cargo audit` 무-플래그 **exit 0 · vulnerabilities 0**)

실측 기준: `cargo-audit 0.22.2` · advisory-db **1,251 advisories** ·
`Cargo.lock` **557 패키지** · **2026-09-19 재측**(`wie-security-audit-schedule-red-three-days-unattended`).
**억제(`--ignore`·`audit.toml`·`deny.toml`) 0건** — 이 0은 억제로 만든 0이 아니다(세 파일 전건 **부재** 확인).

> ★★**2026-09-19 — 이 대장이 처음으로 «되살아난 해소분»을 만났다.** `Security audit`(schedule)이 **3일 연속 red**
> 였고(09-17·18·19), 원인은 새 자문이 아니라 ★**base swap `36df9c31` 이 `Cargo.lock` 을 통째로 갈아
> «우리가 이미 올려 둔 판본»을 되돌린 것**이다(+2,193/−795줄). 실측 — swap «직전» ↔ swap «직후»:
> **`rtrb` 0.3.5 → 0.3.4**(= **C-1** 이 해소한 RUSTSEC-2026-0274 가 **취약점으로 부활**) ·
> **`event-listener` 5.4.2 → 5.4.1**(= **C-2** 가 해소한 RUSTSEC-2026-0221 이 **unsound 경고로 부활**) ·
> `spin` 0.12.2 → 0.12.3(**전진** — C-3 은 무사) · `chacha20` 0.10.0 → 0.10.1(둘 다 yanked — A-4 판정 불변).
> 그리고 swap 이 **`sentry`** 를 들여와 **`rustls` 0.23.44** 가 새로 들어왔고 그것이 RUSTSEC-2026-0285 였다.
> ⇒ ★**「해소했다」는 «그 판본이 lockfile 에 남아 있는 동안만» 참이다** — 아래 C 의 「판정 유효 범위」가
> 겨냥한 것은 «의존성이 올라갈 때»였는데, 실제로 문 것은 ★**«lockfile 이 통째로 교체될 때»** 였다.
> ★**base 를 갈아끼우는 회차는 이 대장의 C 행을 «전건» 재확인하라** — 그것이 이 사건의 재발 방지다.
...
| **A-4** | `chacha20` | **권고 없음**(yanked) | 취약점 아님 — 레지스트리 신호 | `0.10.1`(yanked) → `0.10.2`(2026-08-27) | ★**도달 불가**(3축 · 근거 아래 ⓓ) — 상향은 가능 | 간접(`wie_cli → rodio → rand → chacha20`)이라 `cargo update -p chacha20`. **별건 발권 대상** |

> ★**A-4 의 «설치 버전» 이 `0.10.0` → `0.10.1` 로 움직였다**(2026-09-19 · base swap `36df9c31` 이 옮겼다 —
> 우리가 올린 것이 아니다). ★**ⓓ 의 판정은 그대로 선다** — 그 절이 이미 「`0.10.0`(2026-02-07) · `0.10.1`(2026-06-24)
> **둘 다 yanked**」로 두 판본을 함께 재어 두었고, 비-yanked 최신은 여전히 `0.10.2` 다. ⇒ **처분(별건 발권)도 불변.**
...
### C. 해소분 — **노출 판정 보존** (2026-09-06 신설 `[wie-record-dependency-exposure-verdict-for-next-advisory]`)

> ★★**A 와 목적이 다르다 — 섞지 마라.** A 는 «아직 남아 있는 경고»의 정본이고 그 행 수는 위 절이 적은
> 「경고 수 == 행 수」 불변식에 묶인다. **C 는 «이미 해소된» 자문의 «노출 판정»을 보존하는 자리**이고
> ★**그 불변식 «밖»이다**(해소분은 `cargo audit` 이 더는 출력하지 않는다 — 여기 행이 늘어도 A 는 안 흔들린다).
>
> ★**왜 남기나**: 자문 하나를 만나면 우리가 실제로 답해야 하는 것은 「버전이 패치본인가」가 아니라
...
| **C-4** | RUSTSEC-2026-0285 | `rustls`(간접 — `wie`/`wie-app` → `sentry 0.49.2` → `ureq 3.4.1` → `rustls`) | ★**해소 2026-09-19** — `0.23.44` → **`0.23.45`** = 권고 `Upgrade to >=0.23.45` 경계 도달(`Cargo.lock` 2줄 · `Cargo.toml` 무접촉) | ★**노출 분석을 «하지 않았다» — 버전축으로만 닫았다.** 잰 것은 «범위» 하나뿐이다: ★**배포되는 WASM 아티팩트에는 없다** — `cargo tree -p wie_featurephone --target wasm32-unknown-unknown -i rustls` → **`did not match any packages`**(대조군 `wie-backend` 는 정상 출력 ⇒ 빈 답이 아니라 «부재»다). 도달면은 **네이티브 데스크톱/앱 호스트뿐**이고, TLS 핸드셰이크 경로를 우리가 타는지는 **재지 않았다** | ★**`sentry@0.49.2`/`ureq@3.4.1` 기준의 «경로» 실측이다** — 그 둘을 올리면 다시 재라. ★**도달성 판정이 «없으므로» 이 행을 「안 탄다」의 근거로 인용하지 마라** — 다시 affected 구간으로 내려가면 그때 처음부터 재야 한다 |

> ★★**C-1·C-2 는 «되살아났다가 복원됐다» — 이 표를 「한 번 해소하면 끝」으로 읽지 마라.**
> 2026-09-19 `wie-security-audit-schedule-red-three-days-unattended`:
> base swap `36df9c31` 이 **C-1 `rtrb` 0.3.5 → 0.3.4** · **C-2 `event-listener` 5.4.2 → 5.4.1** 로 되돌렸고,
> 그 회차가 `cargo update -p rtrb --precise 0.3.5` · `-p event-listener --precise 5.4.2` 로 **원래 값에 복원**했다.
> ★**각 행의 「해소 2026-08-08」·「2026-09-08 재확인」은 그대로 사실이다** — 거짓이 된 것은 «그 뒤로도 그렇다»는 «추정»이고,
> 그 추정은 이 표가 아니라 **읽는 사람**이 했다. ⇒ ★**C 행은 «판정»의 보존이지 «현재 lockfile»의 증명이 아니다.**

#### ⓒ C-1 `rtrb` RUSTSEC-2026-0274 — 「목록에 있다」와 「그 경로를 탄다」를 갈랐다

자문 요지: `ReadChunk::commit` 에서 **원소의 `Drop` 이 panic 하면** double free / use-after-free.
★**세 축을 «수»로** 재서 「안 탄다」로 끝냈다(2026-09-06 · `[wie-rustsec-2026-0274-rtrb-double-free-audit-red]`):

⑴ **우리 소스 축** — 워크스페이스 `.rs`/`.toml` 전수에서 `rtrb|ReadChunk` ★**0건**.
   `wie_cli/src/main.rs:26` 이 쓰는 rodio 표면은 `DeviceSinkBuilder`·`Player`·`SamplesBuffer`·
   `SampleTypeConverter` = **재생 전용**이다.
⑵ **rodio 내부 축**(★**판본 의존 — 위 유효 범위 칸이 가리키는 그 축**) — rodio 가 `rtrb` 를 쓰는 곳은
   `src/microphone.rs` **한 파일뿐**인데 그 파일의 `read_chunk|ReadChunk|.commit` ★**0건**
   (실사용은 `pop()`·`slots()`) ⇒ **취약 함수가 애초에 호출되지 않는다.**
⑶ **타입 축** — 원소 타입 `rodio::Sample = Float = f32`(`common.rs:31,43`) = ★**`Drop` 구현 없음**
   ⇒ 자문의 전제(「원소의 `Drop` 이 panic 한다」)가 **구조적으로 성립 불가**.

★**그럼에도 올린 이유**: Constraint 5 가 「no ignores」를 잠그므로 «예외 처리»는 선택지가 아니었고,
rodio 요구가 `^0.3.2` 라 **rodio 를 건드리지 않고** 패치판으로 갈 수 있었다(가장 싼 처분).
⇒ ★**실보안 이득은 0에 가깝다** — 값한 것은 daily red 를 끈 것과, rodio 가 나중에 `read_chunk` 를
쓰더라도 이미 패치판이라는 것뿐이다. **그 사실을 숨기지 않는다.**
