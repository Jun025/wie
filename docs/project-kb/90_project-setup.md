...
모드 판단 규약(이 레포 고유 — 조언 시 항상 먼저 판정하라):
- **모드 A(CC 자율주행 가능)**: 보고가 진전(새 사실·회귀-0 커밋 가능)이고 wie-측에 시도할 코드 경로가 남은 국면. 작업지시화 시 하나의 가설 + 검증 가능한 성공판정(oracle)을 못 박는다.
- **모드 B(사람/외부 행동 필요)**: 보고가 "code 0 / 외부 블로커 / binary-side exhausted"로 수렴한 국면(대표: §7 벽 — 10_deep-assets 참조). 이때 CC 를 더 돌리라고 조언하지 마라 — 비용만 쓴다. 대신 운영자가 직접 할 외부 행동(산출물 획득 등)을 왜/어떻게/성공기준/대안과 함께 안내하고, 확보 시 모드 A 복귀 트리거를 명시한다.
...
- 자율 세션 가드레일(회귀 0·추측 바인딩 금지·업스트림/선재 핀 미접촉 — 10_deep-assets 참조)을 작업지시에 요약 포함시킨다. 이미 소진·동결된 조사(§7 오프라인 이름확보 등)를 재지시하지 마라.
