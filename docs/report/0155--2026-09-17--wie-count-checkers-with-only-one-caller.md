...
**사용자 영향**: 없다(제품 0줄). PR 마다 node 스텝 1개 — ★**러너 실측 1.5~2.0초**
(2 run · `35257783718` 2.04 · `35268371028` 1.49 — ★**같은 코드인데 갈린다. 단일값으로 인용하지 마라**).
★**초판에 적힌 「0.6초」는 «`cargo metadata` 를 부르기 전» 판본의 수다**(run `35234613184`). 근거표는 **§⑺ 게이트**.
...
| ★`35268371028` | ★**`5256998a`**(`-fix2` · 코드 실질 동일) | ★`38 · 0=7 · 1=15 · 2+=16` | 20:02:16.209 → 20:02:17.701 | ★**1.49 s** |

⇒ **0.60 → 1.5~2.0 s**. 늘어난 몫은 `cargo metadata --no-deps` 서브프로세스 **1회**다(이 회차가 「추측 대신
cargo 에 묻는다」로 바꾸며 산 비용이고, 그것이 정확히 초판의 수를 틀리게 만든 원인을 없앤다).
★★**셋째 행은 «이 회차가 자기 교훈을 자기에게 적용한 자리»다** — `-fix2` 가 반려 A 를 닫으며 「**2.04 s**」
한 값을 박았는데, 회신 직전에 돌아온 자기 PR 의 run 이 **같은 코드에서 1.49 s** 를 냈다.
⇒ ★**단일값을 박는 것 자체가 반려 A 와 «같은 병»이다.** 그래서 **범위 + run 번호**로 고쳐 적는다.
★**두 번째·세 번째 행의 차이는 코드가 아니라 러너 부하다**(census 출력이 **완전히 동일**한 것이 그 증거다) —
★**「3.4배」로 인용하지 마라. 참인 것은 «cargo 호출이 붙어 2~3배가 됐다»이고, 그 배수도 잴 때마다 흔들린다.**
