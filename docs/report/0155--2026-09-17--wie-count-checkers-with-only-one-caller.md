...
**사용자 영향**: 없다(제품 0줄). PR 마다 node 스텝 1개 — ★**러너 실측 2.04초**
(head `8a5e9ab5` · run `35257783718`). ★**초판에 적힌 「0.6초」는 «`cargo metadata` 를 부르기 전» 판본의 수다**
(run `35234613184` · 0.60초) ⇒ **3.4배**. 근거표는 **§⑺ 게이트**.
...
| ★`35257783718` | ★**head `8a5e9ab5`**(이 PR 이 착지시킬 코드) | ★`38 · 0=7 · 1=15 · 2+=16` | 18:16:23.350 → 18:16:25.385 | ★**2.04 s** |

⇒ ★**3.4배**. 늘어난 몫은 `cargo metadata --no-deps` 서브프로세스 **1회**다(이 회차가 「추측 대신 cargo 에 묻는다」로
바꾸며 산 비용이고, 그것이 정확히 초판의 수를 틀리게 만든 원인을 없앤다).
