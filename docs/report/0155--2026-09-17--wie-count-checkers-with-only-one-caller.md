...
★★★**[게이트② 반려 승계 `-fix3`] 초판이 여기서 끌어냈던 교훈(「러너 0.6초 = 로컬의 1/4 ⇒ CI 비용은 러너 수를 인용하라」)은
«뒤집힌» 것이 아니라 ★**«물을 수 없는 질문»이었다** — 그리고 `-fix2` 가 그것을 「부등호가 뒤집혔다 · 러너가 더 비싸다」로 적어
★**바로 위 두 줄의 자기 금지를 같은 문단에서 어겼다**(그것이 이 회차의 반려 사유다).
⑴그 문장이 든 **「2.04 s」는 head 의 수가 아니다** — `8a5e9ab5`(직전 핀)의 값이고, ★**위 표가 head `2ab841ca` = 0.64 s 라고 적는다.**
⑵그 참값을 넣으면 러너 **0.64 s** < 로컬 **0.90~1.30 s** ⇒ ★**부등호는 «안» 뒤집혔다.**
⑶그러나 ★**그 방향도 결론이 아니다** — 러너 폭(0.64~2.04)이 로컬 폭(0.90~1.30)을 **품는다**. ⇒ ★**두 자리의 대소는 이 표본으로 «결정되지 않는다».**
⇒ ★★**남는 규율은 방향이 아니라 하나다: 「비용은 «측정한 자리와 커밋»을 밝혀 인용하라 — 그리고 두 자리를 «비교하지» 마라.」**
※`-fix2` 가 여기 적었던 「판정 근거는 **같은 러너·다른 커밋**의 러너 수다」도 함께 **철회한다** —
★그 문장은 같은 커밋에서도 값이 갈린다는 **§⑺ 표 자신의 결론과 모순**이다(같은 회차가 위에서 세우고 아래에서 어겼다).
