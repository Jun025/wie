...
5. 왜 프레임이 남았나 — **게스트는 catch 에서 자기 프레임을 pop 하지 않는다**. 함수 `0x7ac84` 는 7.8KB 본문에
   ...
   ⇒ 게스트 ABI 는 «unwind 가 들어가는 프레임을 소비한다» 이다. 우리 `unwind` 는 pending 만 세우고 프레임을 남겼다.
...
그 이름이 붙지 않은 경로에서는 마을에 남아 계속 돈다. try/catch 를 쓰는 모든 LGT 게임에서, 잡힌 예외 뒤에
나는 무관한 예외가 이미 끝난 함수로 튀지 않는다.
