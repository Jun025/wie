...
### ⑷ 개악 — 3건(회차 2 + 검수자 1). ★첫 번째는 «처음엔 통과했다» · ★세 번째는 «고친 뒤에도 통과했다»
...
★★**[승계 2026-09-23 · 게이트② R2] 위 시험은 «계급»을 약속했는데 실제로는 «한 자리»만 잠갔다.**
검수자가 **자기가 고른** 제품 줄 — 게이트보다 **세 줄 위**의 기록줄 — 에 같은 형상을 넣었고,
`-fix` 회차가 **재현했다**(`15 passed; 0 failed` = ★green):

```
-    outcome.input_steps = input_steps;
+    outcome.input_steps = input_steps_total;
```

★**결과는 원 결함의 완전 복원이고 «더 나쁘다»** — 게이트는 27 을 먹어 영영 발화하지 않고,
JSON 은 0키 런에 `input_steps: 27/27` 을 찍어 ★**사람 눈에도 「다 돌았다」로 보인다**.
★**틀린 것은 단언이 아니라 실패문이 약속한 «범위»다**: 「a gate fed `input_steps_total`
never fires」는 결함 **계급**을 말하는데, 못박힌 것은 그 계급의 **인자 생산 지점 두 곳 중 하나**
(호출부)뿐이었다. ⇒ 같은 `include_str!` 관용으로 **기록줄도 잠갔다**(단언 1줄 추가 · 기존 축 무접촉):

```
---- tests::the_gate_is_handed_the_delivered_count_test stdout ----
assertion `left == right` failed: the delivered count is no longer what feeds the gate —
the record line three lines above the gate must assign `input_steps`, not `input_steps_total`
  left: 0
 right: 1
test result: FAILED. 14 passed; 1 failed
```

★★**그리고 «아직 못 잠근 몫»을 여기와 시험 주석에 «정직하게» 적었다** — 이 저장소의 규율이다.
두 단언은 **게이트에 인자를 건네는 두 줄**을 잠근다. 지역 `input_steps` 가 **어떻게 세어지는지**
(`= 0u64` 초기화 · 디스패치 루프의 `+= 1`)는 잠그지 **않는다**. ★**실측으로 확인했다** —
초기화를 `let mut input_steps = input_steps_total;` 로 바꾸면 두 단언 아래에서도 **green** 이다.
★**그 몫을 문자열로 더 잠그지 «않은» 이유**: 계수 산술을 문자열로 고정하면 결함이 아닌 리팩터에도
red 가 되는 «철자 시험»이 된다. 그 절반은 `zero_injected_steps_is_not_a_pass_test` 의 술어 12행과
러너 블록의 `--inject` 픽스처 2건이 진다.
...
같은 452 JSON 중 **21건**이 PASS 이면서 기본 백스톱(`ticks == 50,000,000`)에 닿았다.

> ★**[정정 2026-09-23 · 게이트② R1 · `-fix` 회차가 452 JSON 전수를 다시 쟀다] 초판의 이 수는 틀렸다.**
> 초판 문안: 「**21건** … **전건** `ms 13.2~17.5초` … 대략 **18~25** / 27 스텝」.
> 집합과 건수는 맞았고(판별자 `result==PASS ∧ ticks==50,000,000` → **21건** · 표본 축소 0),
> ★**틀린 것은 «범위»와 «전건»이다.** 아래가 재측값이다.

| | 초판 서술 | ★재측(452 전수) |
|---|---|---|
| `ms` 범위 | 13.2 ~ **17.5**s | ★**13.176 ~ 19.892**s |
| 절단된 런 | **21 / 21**(전건) | ★**14 / 21** |
| 스텝 범위(절단분) | 약 18 ~ **25** | ★**18 ~ 26** |

★**「전건 절단」이 무너지는 이유는 산수다**: 기본 일정은 첫 키가 `boot_secs 2.5 + 0.3`,
이후 `action_secs 0.6` 간격 27스텝(`wie_validate.rs` 의 `schedule.push` 루프 실측) ⇒
**마지막 키 18.4초** · 데드라인 `t + 1.0` = **20.0초**. 그러므로 ★**`ms ≥ 18.4s` 인 7건은
27스텝을 «전건 전달»받았고 절단이 아니다** — 백스톱에 닿은 것은 일정이 끝난 «뒤»다.
★**그리고 스텝 수는 «상한»이다** — `ms` 는 `run()` 전체를 잰다(`main` 이 `Args::parse()` 직후
`StdInstant::now()` 를 잡고 `run()` 이 «돌아온 뒤» `elapsed()` 를 읽는다) ⇒ zip 읽기 ·
`build_emulator` · 후처리가 전부 포함돼 구동 루프의 벽시계는 그보다 짧다.
⇒ 같은 `ms` 가 가리키는 실제 스텝 수는 여기 적은 값 **이하**다.
※그리고 바로 위 표의 「뒤집히는 2건」도 «상한»이다 — 판별자 `ms < 2,800` 은 그 둘이 `--inject`
런이었는지를 가르지 못한다(구코드 `reason` 이 `--inject` 유무로 갈리지 않는다). 더 엄격한
판별자(`reason` 에 `survived input sequence`)로 재면 `--inject` PASS 집합은 **178건**이고
0스텝은 **0건** — 즉 실제 뒤집힘은 **0~2건**이고 표는 큰 쪽을 적었다.
★**이 정정을 «차단»으로 받은 이유**: 아래 제안 2번(「부분 실행 임계를 데이터에서 고르기」)이
이 표를 **입력**으로 받는다. 임계를 지어내지 않겠다는 자제는 옳지만,
★**그 자제의 값은 넘겨주는 데이터가 맞을 때만 선다.**

⇒ ★**0스텝은 아니지만 그중 14건은 «일정을 다 못 돌고» 「survived input sequence」를 받았다** —
**18 ~ 26 / 27 스텝**(상한). ★**지금은 그 수가 «보인다»**(`input_steps: 22/27`)는 것이 이 회차의 몫이고,
...
- **선재 6쌍** — 내가 «고친» 두 파일 안의 **기존 본문**이다(도구는 수정 파일의 **전문**을 센다):
  `AGENTS.md` 5쌍은 이 도구 자신을 설명하는 문단의 예시어, `wie_validate.rs` 1쌍은 기존 시험 주석어.
  ★**내 diff 가 더한 «코드» 줄에는 하나도 없다.**
- **SUFFIX-ATTACHED 5쌍은 «전건 선재»** — 다섯 다 `AGENTS.md` 의 그 같은 문단이다. ★내 몫의 판단 대상 0.

★**이 문단 자신이 수를 두 번 움직였다**(17/10 → 20/11 → …) — 첫 측정 뒤 산문에 타이틀명을 다시
적었기 때문이다. ★**그것이 이 규약이 겨냥하는 바로 그 실패형**이라, 이 문단에서 stem 을 **한 번도
호명하지 않도록** 고쳐 수렴시킨 뒤 마지막에 다시 쟀다.

★**[승계 2026-09-23 · R1·R2 수정 후 재측]** 세 바구니 **전부 불변**(`B=17/10 · P=2/1 · S=5/5`) —
움직인 것은 트리 digest 뿐이다. 즉 ★**이 승계 회차가 새로 들인 코퍼스 이름은 0** 이다
(R1 은 «수»만 고쳤고 타이틀을 한 번도 호명하지 않았다). 표식은 본문을 다 고친 «뒤에» 갈아끼웠다.
...
<!-- corpus-name-inflow v1 subjects=4 tree=9a0115f3df0ac1e2 B=17/10 P=2/1 S=5/5 -->
