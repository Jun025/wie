...
## ★[`-fix` 2026-09-17] 같은 파일을 만지는 형제 PR — «세어서» 적는다

★★**초판이 「같은 파일을 만지는 열린 PR **0건**」이라 적었고 그것은 «거짓»이었다** — 게이트② F1(major).
그 시점(`16:21:47Z`)에 **#169** 가 **이미 열려 있었고**(`16:02:12Z`) ★**같은 파일을 같은 앵커에서** 고치고 있었다.
★**근인은 오판이 아니라 «세지 않고 선언한 것»이다.** ⇒ 이 절은 **명령과 그 출력**으로 적는다.

```
gh pr list -R Jun025/wie --state open --limit 200 --json number --jq '.[].number'
  → 각 PR 에 gh api repos/Jun025/wie/pulls/<n>/files 로 .github/workflows/engine-contract.yml 포함 여부
02:11 KST 측정: ★#170(이 PR) · ★#171  ── #169 는 그 사이 MERGED(17:10:46Z)
```

| PR | 상태 | `engine-contract.yml` 에서 하는 일 | 순번 주장 |
|---|---|---|---|
| **#169** | ★**MERGED**(`41eb9b64`) | `check-linux-system-deps` 스텝 **추가** | ★**Sixth**(착지 확정) |
| **#170**(이 PR) | OPEN | `check-parked-workflows` 스텝 **추가** | ★**Seventh**(이 `-fix` 가 Sixth→Seventh 로 고쳤다) |
| **#171** | OPEN | ★**주석만**(헤더 · `continue-on-error` 문단) — 추가 스텝 **0** | ★**없음** |

★**착지 순서 = #169(완료) → #170 → #171.** 근거: ⑴#169 는 **이미 착지**해 선택지가 아니다
⑵★**#170 이 «코드 파일»에서 `main` 과 충돌하던 유일한 쪽**이었다(`merge-tree` rc=1 · `engine-contract.yml`) —
이 `-fix` 가 base 를 당겨 **rc=0** 으로 만들었으므로 그 부채를 먼저 턴 쪽이 먼저 착지하는 것이 싸다
⑶#171 은 **추가 스텝이 0** 이라 순번을 다투지 않고, #170 과의 잔여 충돌은 ★**`STATE.md` 뿐**(원장 union = 머지 회차의 몫).
★**순서를 안 정하면**: 셋 중 아무 둘이 착지하는 순간 **코드 파일 충돌**이고 그때는 사람이 푼다.

★★**«나중 것이 순번을 고친다»** — 이 규율이 이 절의 산출물이다. 두 회차가 서로를 모른 채 같은 순번을 주장했고,
★**파일이 자기 순번에 대해 거짓말할 뻔했다.** 지금 파일의 순번은 **Third→Fourth→Fifth→Sixth→Seventh** 로 연속이고
**물리 순서와 일치**한다. ⇒ ★**이 뒤에 스텝을 더하는 회차는 «Eighth» 이고, 그 시점에 다시 세어라.**
