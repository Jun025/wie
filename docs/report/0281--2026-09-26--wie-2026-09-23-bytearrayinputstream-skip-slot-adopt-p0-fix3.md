...
**게임 파일명 유입**(`corpus-name-inflow --corpus` · 브랜치 전체 11파일): 아래 표식의 BOUNDED · SUFFIX-ATTACHED 값 — 전부 주석·문서 속 타이틀명(계측 근거 인용) · 게임 바이트 0. `0273` 의 표식은 이 회차가 브랜치에 파일을 더해 낡아 떼고 이 파일로 옮겼다.
