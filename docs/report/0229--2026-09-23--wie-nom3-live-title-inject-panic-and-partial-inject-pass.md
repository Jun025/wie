...
| | panic n/N | 비고 |
|---|---|---|
| **전**(분기 base `25d0ae69` 무수정 release) | ★**2/8** | 죽은 스텝 `11_LEFT`·`10_OK` — ★**서로 다른데 주소는 전건 `0x104C02B4`** |
| 전(선행 회차 합산 · 티켓 인용) | 5/16 | |
| ★**대조군 — 현행 `origin/main` `24a920c4`**(= 내 수정만 뺀 것) | ★**3/16** | 스텝 `09_UP`·`11_LEFT`·`09_UP` · ★**#261 착지 «뒤»에도 결함이 살아 있다** |
| | | ⇒ 전 합산 ★**10/40 = 25.0%** |
| **후**(분기 base) | ★**0/16** | 전건 `PASS 27/27` |
| **후**(병합 base 재측) | ★**0/28** | 전건 `27/27` |
| | | ⇒ 후 합산 ★**0/44** · 25% 에서 우연 확률 ≈ **0.0000%**(0.75^44) |
...
### ★★base 병합 후 재측 — 그리고 «내 것이 아닌» 실패 하나를 대조군으로 갈랐다

분기 뒤 `origin/main` 이 **3커밋**(#259·#260·#261) 전진했다. ★**#261 은 `java/lang/Object` index 5 =
`notify()` 와 모니터 소유권을 바꾼 엔진 변경**이고, 놈3 의 뜨거운 고리가 정확히 `wait`/`InterruptedException`
이라 **내 측정의 전제를 건드린다**. ⇒ 병합 후 전부 다시 쟀다.

| 측정 | panic | `no frame rendered`(`p0`) | PASS |
|---|---|---|---|
| **내 브랜치**(병합 base) | ★**0 / 28** | 2 / 28 | 26 |
| ★**대조군 `origin/main` `24a920c4`**(내 수정만 뺀 것, 같은 코퍼스·같은 맥) | **3 / 16** | **1 / 16** | 12 |
