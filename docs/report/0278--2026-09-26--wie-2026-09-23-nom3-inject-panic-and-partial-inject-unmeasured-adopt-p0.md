...
**게임 파일명 유입**: `node scripts/corpus-name-inflow.mjs --corpus <코퍼스>` ⇒ BOUNDED 13회/9쌍 — 이 회차가 쓴 것은 관측 대상 타이틀명 1종 4회(문서 3 · worklog 1 · `interface.rs` 의 같은 이름 2회는 기존 줄)뿐이고 나머지는 `interface.rs` 에 **이미 있던** 줄(주석을 고쳐 파일 전체가 대상이 됐다) · SUFFIX-ATTACHED 1회/1쌍 — `interface.rs` 기존 줄의 «더 긴 다른 제목» 1건. 게임 바이트 0.

<!-- corpus-name-inflow v1 subjects=3 tree=484778e9251bf989 B=13/9 P=0/0 S=1/1 -->
