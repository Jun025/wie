...
// truncated where `<stem>.json` is not. The refusal and its numbers are in
// `docs/report/0191`.
//
...
//
// ★A `<platform>/<basename>` third key for that file was measured and refused on
// 2026-09-20 (#227 · `docs/report/0191`): its reasons are truncated where `<stem>.json`
// is not, so joining would classify on less text. The per-run re-measure was removed
// 2026-09-26 (`wie-2026-09-19-adoption-audit-prune`); `git show 978c7b3d:scripts/game-lab-census-map.mjs` has it.
...
const sourceLine =
  ...
  (!summaryRows
    ...
    : summaryUsed
      ...
      : ` ★(summary.tsv has ${summaryRows} row(s) and NONE of them joined — its 'file' column is not corpus-relative (the July baseline stores bare basenames). Falling back to the stem layer, which loses one verdict per colliding stem.)`);
...
const asymmetryLine = collidingStems.length
  ? `★${collidingStems.length} stem(s) exist under 2+ carriers (${collidingStems.length * 2 <= 8 ? collidingStems.join(", ") : `${collidingStems.slice(0, 3).join(", ")}, …`}). A BARE-stem-keyed column gives every copy ONE verdict; the summary.tsv path key and the <bucket>__<stem>.json name (written by scripts/game-lab-recensus.sh since 2026-09-20, for colliding stems only) both tell them apart. Rows from a stem-keyed input are therefore not comparable 1:1 with rows from a path-keyed one — and the July baseline is stem-keyed here (its summary.tsv is basename-keyed; a platform-qualified join was measured and refused — docs/report/0191), so that asymmetry is permanent for it.`
  : null;
