//! Does a key press actually REACH a LGT guest?
//!
...
//! WIPI path is different code (the Clet wrapper card, not the MIDP event queue — LGT hands the key to
//! `CletWrapperCard::keyNotify` after `WIPIKeyCode::from_midp_raw`),
