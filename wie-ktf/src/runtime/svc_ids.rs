...
mod tests {
    ...
    /// Slot number and placeholder name only — the explanatory prose is free to change.
    #[test]
    fn ktf_kernel_extension_message_names_the_slot_and_placeholder() {
        let m = ktf_kernel_extension_message(36, "MC_knlReserved4");
        assert!(m.contains("slot 36") && m.contains("MC_knlReserved4"), "{m}");
    }
    ...
    /// would silently detach the measurement in `docs/report/0178` from the
    /// code it describes.
    #[test]
