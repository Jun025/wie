...
//! boot and binds ~1,400 SVC stubs; `draw_j2me.jar` runs no ARM core; `draw_j2me.zip` fails to
//! load, so nothing runs at all — the zero control.

use std::process::Command;

fn run(fixture: &str) -> String {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../test_data").join(fixture);
